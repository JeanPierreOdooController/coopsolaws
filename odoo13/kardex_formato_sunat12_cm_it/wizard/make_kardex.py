...
class make_kardex_valorado_formato_sunat_v12(models.TransientModel):
	...
	def reportxls_sunat(self):
		...
		attach_id = self.env['ir.attachment'].create({
                    'name': "Kardex Formato Sunat.xlsx",
                    'type': 'binary',
		...
		return {
				'notif_button':{'auto_close':False,'with_menssage':1,'title':'Kardex Formato Sunat Excel','message':'Se proceso el Formato Sunat de '+str(self.fini)+' al ' + str(self.ffin),'eventID':attach_id.id,'model_notify':'ir.attachment','method_notify':'get_download_ls','name_button':'Descargar Kardex Formato Sunat'}
			}	
