...
class AccountAssetRep(models.TransientModel):
	...
	def get_report(self):
		self.env.cr.execute("""
			CREATE OR REPLACE view account_asset_book as ("""+self._get_sql(self.period.date_start,self.period.date_end,self.company_id.id)+""")""")
	...
	def get_excel(self):
		...
		return self.env['popup.it'].get_file('Analisis_de_Depreciaciones.xlsx',base64.encodestring(b''.join(f.readlines())))
