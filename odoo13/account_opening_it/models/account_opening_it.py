...
class AccountOpeningIt(models.Model):
	...
	def preview_opening(self):
		...
		self.env.cr.execute("""select a.account_id,c.currency_id,
							...
							where b.date between '%s' and '%s' and b.company_id=%d and b.state='posted' and c.clasification_sheet in ('0','4') AND a.display_type IS NULL AND a.account_id IS NOT NULL
		...
		for account in accounts:
			...
			FROM (select a.partner_id,a.account_id,a.type_document_id,
			...
			left join
			(select a2.id as move_id,a1.account_id,a1.partner_id,a1.type_document_id,a1.nro_comp,
			...
			group by a2.id, a1.account_id,a1.partner_id,a1.type_document_id,a1.nro_comp, a2.invoice_date
			...
			LEFT JOIN einvoice_catalog_01 ei ON ei.id = T.type_document_id
		...
		return self.env['popup.it'].get_file('Preview - Apertura Contable.xlsx',base64.encodestring(b''.join(f.readlines())))
