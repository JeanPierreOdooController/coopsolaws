...
class AccountDesSummaryRep(models.TransientModel):
	...
	_description = 'Account Des Summary Rep'
	...
	def get_report(self):
		sql = """
			CREATE OR REPLACE view account_des_summary_book as (SELECT row_number() OVER () AS id,a1.* FROM get_summary_destinos('%s',%s) a1)""" % (
	...
	def get_excel(self):
		...
		return self.env['popup.it'].get_file('Resumen_Destino.xlsx',base64.encodestring(b''.join(f.readlines())))