...
class AccountMoveFunctionsEbill(models.Model):
    ...
    def move_anticipo_end(self):
        for record in self:
            len_lines = len(record.invoice_line_ids)
            c  = 0
    ...
    def validate_ebill(self, parameters_ebill, parameters):
        ...
        if (parameters_ebill.verify_amount_odoo or parameters_ebill.verify_amount_odoo == True or parameters_ebill.verify_amount_odoo != False) and not self.is_note_credit_13 and not self.einvoice_id.save_changes:
            ...
            #raise ValidationError(str([round(total_json, 2),round(total_amount, 2)]))
            validacion = False
            #margen = 0

            #cuando ambos son positivos
            if total_json > 0 and total_amount > 0:
                validacion = round(total_json, 2) != round(total_amount, 2)
                #margen = abs(round(total_amount - total_json, 2))

            #cuando son negativos
            if total_json < 0 or  total_amount < 0:
                validacion = total_json != total_amount


            if validacion:
                #raise ValidationError(str([total_json,total_amount]))
    ...
    def body_function_ebill(self, parameters_ebill, parameters):
        ...
        if 1 == 1:
            ...
            for e_line in ebill_lines:
                if self.billing_type == '0':
                    ...
                    if e_line['tipo_de_igv'] in ['2', '3', '4', '5', '6', '7', '10', '11', '12', '13', '14', '15','17']:
        ...
        if self.is_detraccion_anticipo:
            if not int(self.op_type_sunat_id.pse_code) == 4:
                raise ValidationError('EL TIPO DE OPERACION SUNAT : DEBE SER VENTA INTERNA CON ANTICIPOS')
        ...
        if emails.find(',') !=  -1 :
            emails = emails.split(',')
            #raise ValidationError(str(emails[0]))
            try:
                email1 = str(emails[0]).strip()
            except:
                email1 = ''

            try:
                email2 = str(emails[1]).strip()
            except:
                email2 = ''

            try:
                email3 = str(emails[2]).strip()
            except:
                email3 = ''
        else:
            #raise ValidationError('no pesss')
            email1 = emails
        ...
        if self.billing_type == '0':
            ...
            #    raise ValidationError('NO SE CONFIGURO UNA ENTIDAD COMERCIAL PARA EL CLEINTE')
            ebill_json = {
                ...
                "cliente_tipo_de_documento": self.partner_id.commercial_partner_id.l10n_latam_identification_type_id.code_sunat if self.partner_id.commercial_partner_id else self.partner_id.l10n_latam_identification_type_id.code_sunat,
                "cliente_numero_de_documento": self.partner_id.commercial_partner_id.vat if self.partner_id.commercial_partner_id else self.partner_id.vat ,
                "cliente_denominacion": self.partner_id.commercial_partner_id.name if self.partner_id.commercial_partner_id else self.partner_id.name,
                "cliente_direccion": street,
                "cliente_email": email1,
                "cliente_email_1": email2,
                "cliente_email_2": email3,
                ...
                "detraccion": "true" if (self.op_type_sunat_id.code in DetractionCodes or self.is_detraccion_anticipo) else "false",
                "detraccion_tipo": int(
                    self.detraction_type_id.pse_code) if (self.op_type_sunat_id.code in DetractionCodes or self.is_detraccion_anticipo ) else "",
                "detraccion_total": self.detraction_amount if (self.op_type_sunat_id.code in DetractionCodes or self.is_detraccion_anticipo ) else 0,
                "medio_de_pago_detraccion": self.detraction_payment_id.pse_code if (self.op_type_sunat_id.code in DetractionCodes or self.is_detraccion_anticipo ) else "",
    ...
    def generate_lines(self, line, advance_product, advance_regularization, c=None, anticipos=None, c_anticipos=None):
        ...
        error = ''
        free = ['2', '3', '4', '5', '6', '7', '10', '11', '12', '13', '14', '15','17']
        ...
        if self.billing_type == '0':
            ...
            desc = line.name
            if parameters_ebill.print_uom:
                if line.product_uom_id:
                    desc += ' , ' + line.product_uom_id.display_name + ' '
            desc = desc + ' (Gratuita)' if igv_type in free else desc
        ...
        if self.billing_type == '1':
            ...
            if not line.move_advance_line_id:
                ...
                if parameters_ebill.print_uom:
                    if line.product_uom_id:
                        desc += ' , '+line.product_uom_id.display_name+' '
                desc =  desc + ' (Trans.Gratuita)' if igv_type in free else desc
                c_ebill_line = {
                    "unidad_de_medida": 'ZZ' if line.product_id.type == 'service' else 'NIU',
                    "codigo_interno": line.product_id.default_code if line.product_id.default_code else '',
                    "descripcion": desc,
