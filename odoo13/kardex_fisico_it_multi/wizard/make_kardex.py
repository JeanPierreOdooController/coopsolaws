...
class make_kardex(models.TransientModel):
	...
	def do_csvtoexcel(self):
		...
		order by
		almacen_id,product_id,vstf.fecha;
		...
		for line in total_all:
			...
			if almacen == None:
				almacen = (line[14] if line[14] else '')
				producto = (line[13] if line[13] else '')
				saldo = line[11] - line[12]
			elif almacen != (line[14] if line[14] else '') or producto != (line[13] if line[13] else ''):
				almacen = (line[14] if line[14] else '')
				producto = (line[13] if line[13] else '')
				saldo = line[11] - line[12]
			else:
