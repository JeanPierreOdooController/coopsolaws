...
class HrVacation(models.Model):
	...
	def _get_period(self):
		for record in self:
			if record.payslip_run_id.id and record.fiscal_year_id.name:
				periodo = str(record.payslip_run_id.name)
	...
	def set_amounts(self, line_ids, Lot, MainParameter):
		...
		inp_ade_vacation = self.env['hr.payslip.input.type'].search([('company_id', '=', self.env.company.id),('code', '=', 'DESC_VAC')], limit=1)
	...
	def get_vacation(self):
		...
		for Employee in Employees:
			if Employee.contract_id.situation_id.code == '0':
				...
				leave_vacations = self.env['hr.leave'].search([('payslip_run_id', '=', MonthLot.id),
																  ('employee_id', '=', Employee.id),
																  ('work_suspension_id', '=', suspension_type_id.id)], limit=1)

				number_of_days = self.env['hr.leave'].search([('payslip_run_id', '=', MonthLot.id),
																 ('employee_id', '=', Employee.id),
																 ('work_suspension_id', '=', suspension_type_id.id)]).mapped('number_of_days')

				# print("leave_vacations",leave_vacations)
				if leave_vacations.payslip_run_id.id == MonthLot.id:
					...
					# print('compute_date',compute_date,compute_date_final)
					wage = MonthSlip.wage
					# print('wage',wage)
					...
					vals = {
						...
						'days': days,
						'lacks': lacks,
						'record_days': 260 if MonthSlip.contract_id.workday_id.code == 'JOR6' else 210,
		...
		for record in self:
			...
			if membership.is_afp:
				...
				if record.accrued_vacation >= membership.insurable_remuneration:
					afp_si = ReportBase.custom_round(
						membership.prima_insurance / 100 * membership.insurable_remuneration, 2)
				else:
			...
			record.onp = onp
			record.neto_total = ReportBase.custom_round(
				record.total_vacation - afp_jub - afp_si - afp_mixed_com - afp_fixed_com - onp, 2)
