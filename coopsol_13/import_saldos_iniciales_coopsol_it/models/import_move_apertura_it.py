...
class ImportMoveAperturaIt(models.Model):
	...
	def importar(self):
		...
		self.env.cr.execute("""
			...
			left join (select * from einvoice_catalog_01) ec on ec.code = iaa.n_tipo_doc
		...
		for acc in data:
			...
			'amount_untaxed_signed': acc[7]*-1 if self.tipo == 'in' else acc[7],
			'amount_total_signed': acc[7]*-1 if self.tipo == 'in' else acc[7],
			'amount_residual_signed': acc[7]*-1 if self.tipo == 'in' else acc[7],
