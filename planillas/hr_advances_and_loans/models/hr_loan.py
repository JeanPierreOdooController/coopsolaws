...
class HrLoan(models.Model):
	...
	def get_pdf(self):
		...
		if not MainParameter.dir_create_file:
			raise UserError('Falta configurar un directorio de descargas en Parametros Principales')
		route = MainParameter.dir_create_file
		doc = SimpleDocTemplate(route + 'prestamos.pdf', pagesize=letter, rightMargin=40, leftMargin=40, topMargin=40,
								bottomMargin=30)
		elements = []
		...
		data = [
			[I if I else '',
			 ...
			 Paragraph('<strong>Fecha: %s </strong>' % str(datetime.strftime((self.date), '%d-%m-%Y')) or '',
					   style_center)],
		...
		elements.append(
			Paragraph("AUTORIZACION DE DESCUENTOS SOBRE REMUNERACIONES POR PRESTAMO AL TRABAJADOR", style_title))
		elements.append(spacer)
		...
		for line in self.line_ids:
			data.append([
				...
				Paragraph(dict(line._fields['validation'].selection).get(line.validation) if line.validation else '',
						  style_cell)
			])
			y += 1
		...
		return self.env['popup.it'].get_file('Prestamo %s - %s.pdf' % (self.employee_id.name, self.date),
