...
class HrPayslip(models.Model):
	...
	def import_advances(self):
		...
		if not MainParameter.grat_advance_id and not MainParameter.cts_advance_id and not MainParameter.liqui_advance_id and not MainParameter.vaca_advance_id:
			raise UserError('No se ha configurado los tipos de Adelantos en Parametros Generales para los BBSS')
		log = ''
		for record in self:
			sql = """
	...
	def import_loans(self):
		MainParameter = self.env['hr.main.parameter'].get_main_parameter()
		if not MainParameter.grat_loan_id and not MainParameter.cts_loan_id and not MainParameter.liqui_loan_id and not MainParameter.vaca_loan_id:
