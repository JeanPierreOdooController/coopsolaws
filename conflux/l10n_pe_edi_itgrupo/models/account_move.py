...
class AccountMove(models.Model):
    ...
    def _l10n_pe_prepare_dte(self):
        ...
        res['partner_street_address'] = self.partner_id.direccion_complete_it
        if res['invoice_type_code']=='07' or res['invoice_type_code']=='08':
            ...
            if len(self.doc_invoice_relac)>0:
    ...
    def _l10n_pe_prepare_dte_lines(self):
        res = super(AccountMove, self)._l10n_pe_prepare_dte_lines()
        if self.l10n_pe_dte_advance_line:
            line_advance_it = 0
            if self._context.get('counter_advanced_lines', False):
                line_advance_it = self._context.get('counter_advanced_lines')
            res['advance_line'] = self.l10n_pe_dte_advance_line
            res['advance_type'] = "02"
            res['advance_serial'] = self.advance_ids[line_advance_it].serie if line_advance_it in self.advance_ids else res['advance_serial']
            res['advance_number'] = self.advance_ids[line_advance_it].number if line_advance_it in self.advance_ids else res['advance_number']
            res['advance_amount'] = self.l10n_pe_dte_price_base
        return res
