...
class ImportChartAccount(models.TransientModel):
	...
	def create_chart_accounts(self,values):
		...
		if values.get("code") == "":
			raise Warning(_('El campo de code no puede estar vacío.') )
		...
		if account_search:
			#raise UserError('Ya Existe la cuenta %s' % (values.get("code")))
			...
			if values.get("name") == "":
				raise Warning(_('El campo name no puede estar vacío si se va a crear una cuenta.') )

			if values.get("user") == "":
				raise Warning(_('El campo type no puede estar vacío si se va a crear una cuenta.'))
			is_reconcile = False
			is_deprecated= False
			is_destiny = False
			is_document_an = False
	...
	def find_currency(self, name):
		...
		if currency_search:
			...
			if name == "":
				...
				raise Warning(_(' %s Moneda no disponible.') % name)
	...
	def find_tipo_ef(self,tipo_ef):
		...
		if tipo_ef_search:
			...
			if tipo_ef == "":
				...
				raise Warning(_(' %s Tipo Estado Financiero no disponible.') % tipo_ef)
	...
	def find_cuenta(self,cuenta):
		...
		if cuenta_search:
			...
			if cuenta == "":
				...
				raise Warning(_(' %s Cuenta Contable no disponible.') % cuenta)
	...
	def find_tipo_fe(self,tipo_fe):
		...
		if tipo_fe_search:
			...
			if tipo_fe == "":
				...
				raise Warning(_(' %s Tipo Flujo de Efectivo no disponible.') % tipo_fe)
		...
		if tax_ids:
			if ';' in  tax_ids:
				...
				for name in tax_names:
					...
					if not tax:
						raise Warning(_('%s No existe el impuesto en su sistema') % name)
					for t in tax:
						tax_ids.append(t)
				...
				if not tax:
					raise Warning(_('"%s" No existe el impuesto en su sistema') % tax_names)
				for t in tax:
					tax_ids.append(t)
		return tax_ids
		...
		if tag_ids:
			if ';' in  tag_ids:
				...
				for name in tag_names:
					...
					if not tag:
						raise Warning(_('"%s" No existe el etiqueta en su sistema') % name)
					tag_ids.append(tag)
				...
				if not tag:
					raise Warning(_('"%s" No existe el etiqueta en su sistema') % tag_names)
				tag_ids.append(tag)
