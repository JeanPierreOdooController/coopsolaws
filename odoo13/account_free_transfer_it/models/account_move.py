...
class AccountMove(models.Model):
	...
	def post(self):
		...
		for move in self:
			if move.type == 'out_invoice':
				free_transer_tax_ids = self.env['main.parameter'].search([('company_id','=',self.company_id.id)],limit=1).free_transer_tax_ids
	...
	def make_move_free_transfer(self,lines):
		m = self.env['main.parameter'].search([('company_id','=',self.company_id.id)],limit=1)
