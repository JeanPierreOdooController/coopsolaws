...
class NetPatrimonyWizard(models.TransientModel):
	...
	_description = 'Account Patrimony Wizard'
	...
	def _get_net_patrimony_sql(self):
		sql = """
			...
			a1.move_id,
			SUM(CASE WHEN apt.code='001' THEN -a1.balance ELSE 0 END) AS capital,
			SUM(CASE WHEN apt.code='002' THEN -a1.balance ELSE 0 END) AS acciones,
			SUM(CASE WHEN apt.code='003' THEN -a1.balance ELSE 0 END) AS cap_add,
			SUM(CASE WHEN apt.code='004' THEN -a1.balance ELSE 0 END) AS res_no_real,
			SUM(CASE WHEN apt.code='005' THEN -a1.balance ELSE 0 END) AS exce_de_rev,
			SUM(CASE WHEN apt.code='006' THEN -a1.balance ELSE 0 END) AS reservas,
			SUM(CASE WHEN apt.code='007' THEN -a1.balance ELSE 0 END) AS res_ac,
