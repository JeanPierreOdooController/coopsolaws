...
class hr_quincenales(models.Model):
	...
	partner_id = fields.Many2one('res.partner', string='Cliente', related='payslip_run_id.partner_id', store=True)
	...
	def generate(self):
		...
		he = self.env['hr.employee'].search([('contract_ids.state', 'in', ('open', 'close')),('contract_id.structure_type_id', '=', structure_type_id),
											 ('contract_id.partner_id', '=', self.partner_id.id), ('company_id', '=', self.env.company.id)])
		...
		for contrato in contracts:
			...
			vals = {
				...
				'movilidad':contrato.movilidad,
				'refrigerio':contrato.refrigerio,
				'bono_cump':contrato.bono_cump,
				'bono_proy':contrato.bono_proy,
				'bono_prod':contrato.bono_prod,
				'onp':onp,
	...
	def generar_excel(self):
		...
		rc = self.env.company
		worksheet.merge_range('A1:K1', rc.name if rc.name else '', title_format)
		worksheet.merge_range('A2:K2', rc.partner_id.l10n_latam_identification_type_id.name+': ' + rc.partner_id.vat, title_format)
		headers = [
		...
		for data in self.quincenales_lines:
			...
			col += 1
			worksheet.write(row, col, data.movilidad if data.movilidad else 0, numeric_format)
			col += 1
			worksheet.write(row, col, data.refrigerio if data.refrigerio else 0, numeric_format)
			col += 1
			worksheet.write(row, col, data.bono_cump if data.bono_cump else 0, numeric_format)
			col += 1
			worksheet.write(row, col, data.bono_proy if data.bono_proy else 0, numeric_format)
			col += 1
			worksheet.write(row, col, data.bono_prod if data.bono_prod else 0, numeric_format)
			col += 1
			...
			col += 1
			worksheet.write(row, col, data.income if data.income else 0, numeric_format)
			col += 1
			worksheet.write(row, col, data.expenses if data.expenses else 0, numeric_format)
			col += 1
		...
		worksheet.set_column('C:V', col_sizes[2])
		workbook.close()
...
class hr_quincenales_lines(models.Model):
	...
	movilidad = fields.Float('Movilidad', tracking=True)
	refrigerio = fields.Float('Refrigerio', tracking=True)
	bono_cump = fields.Float('Bono Cump', tracking=True)
	bono_proy = fields.Float('Bono Proy', tracking=True)
	bono_prod = fields.Float('Bono Produc', tracking=True)
	...
	@api.depends('basico', 'asignacion_familiar','movilidad', 'refrigerio','bono_cump', 'bono_proy','bono_prod')
	def compute_total(self):
		for l in self:
			res = l.basico + l.asignacion_familiar + l.movilidad + l.refrigerio + + l.bono_cump + l.bono_proy + l.bono_prod - l.onp - l.afp_com - l.afp_prima - l.afp_jub - l.quinta_cat
	...
	@api.depends('monto', 'income','expenses')
