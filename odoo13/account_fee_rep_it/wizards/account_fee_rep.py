...
class AccountFeeRep(models.TransientModel):
	...
	def get_report(self):
		self.domain_dates()
		self.env.cr.execute("""
			CREATE OR REPLACE view account_fee_book as ("""+self._get_sql(self.date_ini,self.date_end,self.company_id.id,self.currency,self.type_date)+""")""")
	...
	def get_plame(self,type,x_date_ini,x_date_end,x_company,x_type_date):

		self.env.cr.execute("""
			CREATE OR REPLACE view account_fee_book as ("""+self._get_sql(x_date_ini,x_date_end,x_company.id,'pen',x_type_date)+""")""")
	...
	def _get_sql(self,x_date_ini,x_date_end,x_company_id,x_mon,x_date_type):

		if x_date_type == 'date':
			sql_date = "(tt.fecha_doc between '%s' and '%s')"%(x_date_ini.strftime('%Y/%m/%d'),x_date_end.strftime('%Y/%m/%d'))
		elif x_date_type == 'payment_date':
			sql_date = "(tt.fecha_p between '%s' and '%s')"%(x_date_ini.strftime('%Y/%m/%d'),x_date_end.strftime('%Y/%m/%d'))
		else:
			sql_date = "(am.invoice_date_due between '%s' and '%s')"%(x_date_ini.strftime('%Y/%m/%d'),x_date_end.strftime('%Y/%m/%d'))
		...
		if x_mon == 'pen':
			sql = """select 
				...
				where %s
			""" % (x_date_type,x_company_id,sql_date)
		else:
			sql = """select 
				tt.id,
				tt.periodo,
