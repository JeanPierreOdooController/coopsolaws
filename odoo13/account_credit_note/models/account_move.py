...
class AccountMove(models.Model):
	...
	def make_credit_note_it(self):
		for i in self:
			if i.type not in ('in_refund','out_refund'):
			...
			if i.invoice_payment_state in ('paid'):
			...
			vals = (0,0,{
				...
				'tc': filtered_line.tc if i.currency_id != self.env.company.currency_id else 1,
				'date_maturity':False,
			...
			for doc in lines:
				...
				if doc.amount_currency != 0:
					amount_cy = doc.amount_currency * -1  if i.type == 'out_refund' else doc.amount_currency
				...
				'debit': doc.amount if i.type == 'in_refund' else 0,
				'credit': doc.amount if i.type == 'out_refund' else 0,
				'tc': abs(doc.amount)/abs(amount_cy) if doc.amount_currency != 0 else 1,
				'date_maturity':False,
				'company_id': i.company_id.id,
				'reconciled':False,
				})
			...
			credit_journal = self.env['main.parameter'].search([('company_id','=',i.company_id.id)],limit=1).credit_journal
