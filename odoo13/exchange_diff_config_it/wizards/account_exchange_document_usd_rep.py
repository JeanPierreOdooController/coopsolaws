...
class AccountExchangeDocumentUsdRep(models.TransientModel):
	...
	_description = 'Account Exchange Document USD Rep'
	...
	def get_report(self):
		self.env.cr.execute("""
			DROP VIEW IF EXISTS account_exchange_document_book;
	...
	def do_invoice(self):
		...
		sql = """SELECT smd.*,ei.id AS type_document_id FROM get_saldos_me_documento_final('%s','%s',%s) smd
				 LEFT JOIN einvoice_catalog_01 ei ON ei.code = smd.td_sunat
				 WHERE smd.saldome = 0""" % (str(self.fiscal_year_id.name),str(self.period.code),str(self.company_id.id))
		...
		for elemnt in obj:
			vals = (0,0,{
				...
				'debit': 0 if elemnt[11] > 0 else abs(elemnt[11]),
				'credit': 0 if elemnt[11] < 0 else abs(elemnt[11]),
				'amount_currency': 0,
				'currency_id': currency.id,
				'type_document_id': elemnt[13],
				'nro_comp': elemnt[3],
				'tc':1,
				'company_id': self.company_id.id,
			})
			sum_credit+= 0 if elemnt[11] < 0 else abs(elemnt[11])
			sum_debit+= 0 if elemnt[11] > 0 else abs(elemnt[11])
			lineas.append(vals)
		...
		move_id.action_concile_special()
	...
	def get_excel(self):
		...
		worksheet = workbook.add_worksheet("DIFERENCIAS PAGADAS")
		...
		return self.env['popup.it'].get_file('Diferencias Pagadas.xlsx',base64.encodestring(b''.join(f.readlines())))

	def _get_sql_report(self,fiscal_year,period,company_id):
		sql_partner_adjustment = ""
		partner_adjustment_id = self.env['main.parameter'].search([('company_id','=',self.company_id.id)],limit=1).partner_adjustment_id
		
		if partner_adjustment_id:
			sql_partner_adjustment = "AND vst.partner <> %d"%(partner_adjustment_id.id)
		...
		sql = """SELECT 
				...
				LEFT JOIN res_partner rp ON rp.id = vst.partner
				WHERE vst.saldome = 0 %s
			""" % (period.code,
				period.id,
				fiscal_year,
				period.code,
