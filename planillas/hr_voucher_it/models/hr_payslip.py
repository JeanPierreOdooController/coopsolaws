...
class HrPayslip(models.Model):
	...
	def get_vouchers(self, payslips=None):
		MainParameter = self.env['hr.main.parameter'].get_main_parameter()
		if MainParameter.type_boleta == '1':
			doc = SimpleDocTemplate(MainParameter.dir_create_file + 'Boleta.pdf', pagesize=letter, rightMargin=30,
									leftMargin=30, topMargin=30, bottomMargin=20)
			elements = []
			if payslips:
	...
	def generate_voucher(self):
		...
		INCOME = self.line_ids.filtered(
			lambda sr: sr.category_id.id in MainParameter.income_categories.ids and sr.total > 0)
		DISCOUNTS = self.line_ids.filtered(
			lambda sr: sr.category_id.id in MainParameter.discounts_categories.ids and sr.total > 0)
		CONTRIBUTIONS = self.line_ids.filtered(
			lambda sr: sr.category_id.id in MainParameter.contributions_categories.ids and sr.total > 0)
		CONTRIBUTIONS_EMP = self.line_ids.filtered(
			lambda sr: sr.category_id.id in MainParameter.contributions_emp_categories.ids)
		...
		data = [
			[Paragraph('RUC: %s' % self.company_id.vat or '', style_cell),
			 ...
			 Paragraph('Periodo: %s - %s' % (datetime.strftime((self.date_from), '%d-%m-%Y') or '',
											 datetime.strftime((self.date_to), '%d-%m-%Y') or ''), style_cell)],
		]
		...
		y = 0
		memoria = []
		for line in Contract.work_suspension_ids.filtered(
				lambda payslip: payslip.payslip_run_id.id == self.payslip_run_id.id):
			# print("line",line.suspension_type_id)
			if line.suspension_type_id.code in memoria:
				continue
			total_dias = self.env['hr.work.suspension'].search(
				[('payslip_run_id', '=', self.payslip_run_id.id), ('contract_id', '=', Contract.id),
		...
		t = Table(first_row + second_row + third_row + fourth_row + fifth_row, 8 * internal_width,
				  (y + 11) * [0.5 * cm])
		t.setStyle(TableStyle(
			first_row_format + second_row_format + third_row_format + fourth_row_format + fifth_row_format + global_format))
		elements.append(t)
		elements.append(spacer)
		...
		for i in SRC:
			...
			for line in SRC[i]:
				data += [[
					...
					Paragraph('{:,.2f}'.format(line.total) or '0.00' if line.category_id.type == 'in' else '',
							  style_right),
					Paragraph('{:,.2f}'.format(line.total) or '0.00' if line.category_id.type == 'out' else '',
							  style_right), ''
				]]
				y += 1
		y += 1
		data += [[Paragraph(NET_TO_PAY.salary_rule_id.name or '', style_left), '', '', '',
				  Paragraph('{:,.2f}'.format(NET_TO_PAY.total) or '0.00', style_right)]]
		data_format += [
			('SPAN', (0, y), (3, y)),
			('BACKGROUND', (0, y), (-1, y), bg_color),
			('ALIGN', (0, 0), (-1, -1), 'CENTER'),
	...
	def generate_voucher_v2(self, objeto_canvas):
		...
		objeto_canvas.drawCentredString((wReal / 2) + 20, hReal - 42,
										"PLANILLA %s" % self.payslip_run_id.name or '')
		...
		data = [
			[Paragraph('<strong>Trabajador</strong>', style_left),
			 Paragraph(': %s' % Employee.name.title() or '', style_left),
			 Paragraph('<strong>Fecha de Ingreso</strong>', style_left),
			 Paragraph(': %s' % str(datetime.strftime((admission_date), '%d-%m-%Y')) or '', style_left),
			 Paragraph('<strong>Dias Lab</strong>', style_left), Paragraph(': %d' % DLAB or '', style_left)],
			[Paragraph('<strong>Tipo Trab</strong>', style_left),
			 Paragraph(': %s' % Contract.worker_type_id.name.capitalize() if Contract.worker_type_id.name else '',
					   style_left),
			 Paragraph('<strong>Fecha de Cese</strong>', style_left),
			 Paragraph(': %s' % datetime.strftime(date_end, '%d-%m-%Y') if date_end else '', style_left),
			 Paragraph('<strong>Dias Subs</strong>', style_left), Paragraph(': %d' % DIA_SUB or '0', style_left)],
			[Paragraph('<strong>Area</strong>', style_left),
			 Paragraph(': %s' % Contract.department_id.name.capitalize() if Contract.department_id.name else '',
					   style_left),
			...
			[Paragraph('<strong>Cargo</strong>', style_left),
			 Paragraph(': %s' % Contract.job_id.name.capitalize() if Contract.job_id.name else '', style_left),
			 Paragraph('<strong>&nbsp; &nbsp;&nbsp; &nbsp; Inicio Vac</strong>', style_left),
			 Paragraph(': %s' % datetime.strftime((fecha_ing_vac), '%d-%m-%Y') if fecha_ing_vac != '' else '',
					   style_left),
			 Paragraph('<strong>Dias Vac</strong>', style_left), Paragraph(': %d' % dias_vaca or '0', style_left)],
			[Paragraph('<strong>Centro de Costos</strong>', style_left), Paragraph(
				': %s' % self.distribution_id.description.capitalize() if self.distribution_id.description else self.distribution_id.name,
				style_left),
			 Paragraph('<strong>&nbsp; &nbsp;&nbsp; &nbsp; Fin Vac</strong>', style_left),
			 Paragraph(': %s' % datetime.strftime((fecha_fin_vac), '%d-%m-%Y') if fecha_fin_vac != '' else '',
					   style_left),
			 Paragraph('<strong>N° Horas Ord</strong>', style_left),
			 Paragraph(': %s' % str(ReportBase.custom_round(DLAB_DEC_INT[1])) or '0', style_left)],
			[Paragraph('<strong>Tipo de Docum</strong>', style_left), Paragraph(': %s <strong>Nro.</strong> %s' % (
			Employee.type_document_id.name or '', Employee.identification_id or ''), style_left),
			 Paragraph('<strong>Reg Pensionario</strong>', style_left), Paragraph(
				': %s' % self.membership_id.name.title() if self.membership_id.name else Contract.membership_id.name,
				style_left),
			 Paragraph('<strong>N° Horas Ext</strong>', style_left),
			 Paragraph(': %s' % str(ReportBase.custom_round(EXT_DEC_INT[1])) or '', style_left)],
			[Paragraph('<strong>Regimen Laboral</strong>', style_left),
			 Paragraph(': %s' % dict(self._fields['labor_regime'].selection).get(self.labor_regime) or '', style_left),
			 Paragraph('<strong>C.U.S.P.P.</strong>', style_left),
			 Paragraph(': %s' % Contract.cuspp if Contract.cuspp else '', style_left),
			 Paragraph('<strong>Rem Basica</strong>', style_left),
			 Paragraph(': {:,.2f}'.format(self.wage) or '0.00', style_left)],
		...
		for descuento in CONTRIBUTIONS:
			...
			first_pos += size_widths[0]
			objeto_canvas.drawRightString(first_pos + 80, h_des,
										  '{:,.2f}'.format(descuento.total) if descuento.total else '0.00')
			first_pos += size_widths[1]
		...
		for aporte in CONTRIBUTIONS_EMP:
			...
			first_pos += size_widths[0]
			objeto_canvas.drawRightString(first_pos + 50, h_apor,
										  '{:,.2f}'.format(aporte.total) if aporte.total else '0.00')
			first_pos += size_widths[1]
		...
		I = ReportBase.create_image(MainParameter.signature, MainParameter.dir_create_file + 'signature.jpg', 160.0,
									35.0)
		data = [
			[I if I else '', ''],
		...
		objeto_canvas.setFont("Helvetica-Bold", 10)
		objeto_canvas.drawCentredString((wReal / 2) + 20, hReal - 457,
										"PLANILLA %s" % self.payslip_run_id.name or '')
		...
		for ingreso in INCOME:
			...
			objeto_canvas.drawString(first_pos, h_ing, particionar_text(ingreso.name if ingreso.name else '', 110))
			first_pos += size_widths[0]
			objeto_canvas.drawRightString(first_pos + 80, h_ing,
										  '{:,.2f}'.format(ingreso.total) if ingreso.total else '0.00')
			first_pos += size_widths[1]
