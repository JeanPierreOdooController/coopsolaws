...
class AccountDetractionsWizard(models.TransientModel):
	...
	_description = 'Import Detractions Wizard'
	...
	def generar(self):
		...
		if not invoice.code_operation:
			raise UserError(u"Codigo de Operación Obligatorio al generar la detracción")
		flag_ver = True
		data = {
		...
		if len(filtered_line)>1:
			raise UserError('Si uso Plazos de pago con diferentes días debe generar la Detracción por Pagos Múltiples Advance.')
		if invoice.type == 'in_invoice':
			...
			if invoice.currency_id.name == 'USD':
				line_cc = (0,0,{
					...
					'debit': round(self.monto * invoice.currency_rate),
					'credit':0,
				...
				line_cc = (0,0,{
					...
					'debit': 0,
					'credit':round(self.monto * invoice.currency_rate),
					'name':'DETRACCION - '+invoice.ref,
					'partner_id': invoice.partner_id.id,
					'nro_comp': invoice.ref,
		...
		if tt.state =='draft':
			tt.post()
		tt.code_operation = invoice.code_operation
