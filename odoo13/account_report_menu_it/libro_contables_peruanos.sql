...
CREATE OR REPLACE FUNCTION public.get_diariog(
	...
	date_to date,
	company_id integer)
	RETURNS TABLE(periodo text, fecha date, libro character varying, voucher character varying, cuenta character varying, debe numeric, haber numeric, balance numeric, moneda character varying, 
	...
	ple_ventas character varying, journal_id integer, account_id integer, partner_id integer, registro character varying, move_id integer, move_line_id integer, analytic_account_id integer) AS
	$BODY$
	BEGIN
	RETURN QUERY 
	SELECT
	...
	ROWS 1000;
----------------------------------------------------------------------------------------------------------------------
DROP FUNCTION IF EXISTS public.get_correccion_diariog(date, date, integer) CASCADE;

CREATE OR REPLACE FUNCTION public.get_correccion_diariog(
	date_from date,
	date_to date,
	company_id integer)
	RETURNS TABLE(periodo text, fecha date, libro character varying, voucher character varying, cuenta character varying, debe numeric, haber numeric, balance numeric, moneda character varying, 
    ...
    FROM account_move a1
        ...
        LEFT JOIN (SELECT ARRAY_AGG(aatamlr.account_analytic_tag_id) as analytic_tag_ids,
                ...
                LEFT JOIN account_analytic_tag aat ON aat.id = aatamlr.account_analytic_tag_id
                GROUP BY aatamlr.account_move_line_id) a13 ON a13.account_move_line_id = a2.id
    WHERE a1.state::text = 'posted'::text AND a2.display_type IS NULL AND a2.account_id IS NOT NULL AND (a1.date_corre_ple::date BETWEEN $1 and $2) AND a1.company_id = $3
    ORDER BY (date_part('month'::text, a1.date)), a3.code, a1.name, a2.debit DESC, a4.code;
	END;
	$BODY$
	LANGUAGE plpgsql VOLATILE
   ...
   FOR var_r IN(
        select * from (
           SELECT * FROM VST_BANCOS WHERE VST_BANCOS.fecha>= fec_ini 
            ...
            FROM VST_BANCOS WHERE VST_BANCOS.fecha < fec_ini AND VST_BANCOS.company_id = id_compannia
...
sum(T.debe) as cargo,sum(T.haber) as abono,null as tc,sum(coalesce(T.importe_me,0)) as importedivisa,null as asiento
...
CREATE OR REPLACE FUNCTION public.get_caja_bancos(
    ...
    IN fec_fin date,
    IN id_compannia integer)
  RETURNS TABLE(periodo character varying, fecha date, libro character varying, voucher character varying, cuenta character varying, debe numeric, 
  debe_me numeric, haber numeric, haber_me numeric, balance numeric, balance_me numeric, moneda character varying, tc numeric, 
  code_cta_analitica character varying, glosa character varying, td_partner character varying, doc_partner character varying, 
  partner character varying, td_sunat character varying, nro_comprobante character varying, fecha_doc date, fecha_ven date, 
  company_id integer, account_id integer, saldo numeric, saldo_me numeric) AS
$BODY$
DECLARE 
    var_r record;
    quiebre TEXT;
   ...
   FOR var_r IN(
        select * from (
           SELECT * FROM VST_CAJA_BANCOS WHERE VST_CAJA_BANCOS.fecha>= fec_ini 
            ...
            FROM VST_CAJA_BANCOS WHERE VST_CAJA_BANCOS.fecha < fec_ini AND VST_CAJA_BANCOS.company_id = id_compannia
...
CREATE OR REPLACE VIEW public.vst_compras_1_1 AS 
 SELECT row_number() OVER () AS id,
   ...
   FROM ( SELECT "left"(to_char(am.date::timestamp with time zone, 'yyyymmdd'::text), 6) AS periodo,
            ...
            AS serie,
            CASE
                WHEN ec1.code in ('50','52') THEN to_char(am.invoice_date , 'yyyy')
                ELSE NULL
            END AS anio,
            CASE
...
BEGIN
 ...
 SELECT row_number() OVER () AS id,
   ...
   FROM ( SELECT "left"(to_char(am.date::timestamp with time zone, 'yyyymmdd'::text), 6) AS periodo,
            ...
            am.invoice_date AS fecha_e,
            CASE WHEN am.invoice_payment_state = 'paid' THEN am.invoice_date_due ELSE NULL END AS fecha_p,
            ec1.code AS td,
                CASE
...
CREATE OR REPLACE VIEW public.vst_destinos AS 
 SELECT T.* FROM(SELECT a1.periodo,
    ...
    a1.voucher,
    a1.cuenta,
    round(a1.debe,2) as debe,
    round(a1.haber,2) as haber,
    a1.balance,
...
UNION ALL
 SELECT b7.periodo,
    ...
    b7.cuenta,
        CASE
            WHEN b1.amount < 0::numeric THEN round(abs(b1.amount),2)
            ELSE 0::numeric
        END AS debe,
        CASE
            WHEN b1.amount > 0::numeric THEN round(b1.amount,2)
            ELSE 0::numeric
        END AS haber,
    b7.balance,
...
round(sum(vst_d.debe),2) as debe,
0:: numeric as haber
from vst_destinos vst_d
where vst_d.periodo = $1 and vst_d.company_id = $2
...
0::numeric as debe,
round(sum(vst_d.debe),2) as haber
from vst_destinos vst_d
where vst_d.periodo = $1 and vst_d.company_id = $2
...
0::numeric as debe,
round(sum(vst_d.haber),2) as haber
from vst_destinos vst_d
where vst_d.periodo = $1 and vst_d.company_id = $2
group by vst_d.des_debe
...
select vst_d.des_haber as cuenta,
round(sum(vst_d.haber),2) as debe,
0::numeric as haber
from vst_destinos vst_d
where vst_d.periodo = $1 and vst_d.company_id = $2
...
------------------------------------------account_destinos_rep_it---------------------------------------------------------------
------------------------------------NUEVA FUNCION MEJORADA PARA ODOO15------------------------------------------------------------
...
CREATE OR REPLACE FUNCTION public.get_mayorg(
  ...
  account_ids integer[])
	RETURNS TABLE(periodo character varying, fecha date, libro character varying, voucher character varying, cuenta character varying, debe numeric, 
  debe_me numeric, haber numeric, haber_me numeric, balance numeric, balance_me numeric, moneda character varying,
	tc numeric, code_cta_analitica character varying, glosa character varying, td_partner character varying, doc_partner character varying, 
	partner character varying, td_sunat character varying, nro_comprobante character varying, fecha_doc date, fecha_ven date, account_id integer, 
    move_id integer, move_line_id integer, saldo numeric, saldo_me numeric) AS
	$BODY$
	BEGIN
	RETURN QUERY 
select 
...
rc.name as moneda,
aml.tc,
ana.code as code_cta_analitica,
am.glosa,
...
null as move_id,
0 as move_line_id,
(EXTRACT (YEAR FROM date_from)::character varying ||'/01/01')::date as fecha, 
aml.account_id,
...
sum(coalesce(aml.balance,0)) as balance,
...
LEFT JOIN einvoice_catalog_01 lldt ON lldt.id = aml.type_document_id
