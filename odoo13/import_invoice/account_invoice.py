...
class gen_inv(models.TransientModel):
		...
		if invoice_search:
			...
			if self.type == "in":
				...
				if partner_id.property_account_receivable_id:
					...
					account_search = self.env['ir.property'].search([('name', '=', 'property_account_receivable_id'),('company_id','=',self.env.company.id)],limit=1)
				...
				if partner_id.property_account_payable_id:
					...
					account_search = self.env['ir.property'].search([('name', '=', 'property_account_payable_id'),('company_id','=',self.env.company.id)],limit=1)
					account_id = account_search.value_reference
					if not account_id:
	...
	def make_invoice_line(self, values, inv_id):
		...
		if self.import_prod_option == 'barcode':
			product_id = product_obj.search([('barcode',  '=',values['product'])])
		elif self.import_prod_option == 'code':
			product_id = product_obj.search([('default_code', '=',values['product'])])
		else:
			product_id = product_obj.search([('name', '=',values['product'])])
		...
		if self.account_opt == 'default':
			if inv_id.type == 'out_invoice':
				if product_id and product_id.property_account_income_id:
					...
					account_search = self.env['ir.property'].search([('name', '=', 'property_account_income_categ_id'),('company_id','=',self.env.company.id)],limit=1)
					account = self.env['account.account'].browse(int(account_search.value_reference.split(",")[1]))
