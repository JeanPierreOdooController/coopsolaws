...
class HrMainParameter(models.Model):
	...
	def calculate_bonus(self, admission_date, date_from, months, Lines):
		...
		for key, value in Codes.items():
			if months >= 3 and value >= 3:
				amount = sum(Lines.filtered(lambda line: line.code == key).mapped('amount'))
	...
	def compute_benefits(self, record, record_type, liquidation=False):
		...
		if liquidation:
			...
			payslip_month = record.resumen_plani_id.date_start.month
		...
		if liquidation:
			MonthLot = record.resumen_plani_id
			...
			Employees = FilteredSlips.filtered(lambda slip: slip.contract_id.labor_regime in ['general', 'small'] and
																...
																slip.periodo_id.date_end >= slip.contract_id.date_end and
											   					slip.contract_id.partner_id.id == record.partner_id.id).mapped('employee_id')
		else:
			MonthLot = self.env['hr.resumen.planilla'].search([('date_start', '>=', month_date_from), ('date_end', '<=', month_date_to)])
			if record_type in ['05', '11']:
				FilteredSlips = MonthLot.slip_ids.filtered(lambda slip: not slip.contract_id.less_than_four)
			else:
				FilteredSlips = MonthLot.slip_ids
			if record.worker_type_id:
				Employees = FilteredSlips.filtered(lambda slip: slip.contract_id.labor_regime in ['general', 'small'] and
																slip.contract_id.situation_id.code != '0' and
											   					slip.contract_id.worker_type_id.id == record.worker_type_id.id and
											   					slip.contract_id.partner_id.id == record.partner_id.id).mapped('employee_id')
			else:
				Employees = FilteredSlips.filtered(lambda slip: slip.contract_id.labor_regime in ['general', 'small'] and
																slip.contract_id.situation_id.code != '0' and
		...
		Lots = self.env['hr.resumen.planilla'].search([('date_start', '>=', date_from), ('date_start', '<=', date_to)])
		...
		for Employee in Employees:
			...
			if record_type == '12':
				...
				Lots_wd = self.env['hr.resumen.planilla'].search([('date_start', '>=', date_from_wd),('date_start', '<=', date_to_wd)])
				for Lot in Lots_wd:
					...
					EmployeeSlips = Lot.slip_ids.filtered(lambda slip: slip.employee_id == Employee and
																	   slip.periodo_id.date_start >= admission_payslip_date and
																	   slip.periodo_id.date_end <= MonthSlip.resumen_plani_id.date_end)
					...
					# if WorkingWD == Lot.date_end.day:
					# print("Lot.date_end.month",Lot.date_end.month)
				...
				for Lot in Lots:
					...
					WorkedDays = EmployeeSlips.mapped('slip_wd_ids')
					WorkingWD =	sum(WorkedDays.filtered(lambda line: line.wd_type_id in MainParameter.working_wd_ids).mapped('number_of_days'))
					# print("else date_end.month", Lot.date_end.month)
					if Lot.date_end.month == 2 and WorkingWD >= 28:
						months += 1
					elif WorkingWD >= 30:
						months += 1
					...
					# print("WorkingWD",WorkingWD)
					# print("months",months)
					Commissions += SalaryRules.filtered(lambda line: line.salary_rule_id in MainParameter.commission_sr_ids and line.amount > 0)
					Bonus += SalaryRules.filtered(lambda line: line.salary_rule_id in MainParameter.bonus_sr_ids and line.amount > 0)
					ExtraHours += SalaryRules.filtered(lambda line: line.salary_rule_id == MainParameter.extra_hours_sr_id and line.amount > 0)

			if Employee.contract_id.worker_type_id.code == '20':
				months += days//30
				days = days%30
			...
			if record_type in ['07', '12']:
				...
				vals['total'] = total
				if liquidation:
					Grat = self.env['hr.gratification'].search([('resumen_plani_id', '=', liquidation.resumen_plani_id.id),
				...
				cts_soles = ReportBase.custom_round(cts_per_month + cts_per_day - amount_per_lack + remaining_wage, 2)
				total_cts = cts_soles
				...
				if liquidation:
					CTS = self.env['hr.cts'].search([('resumen_plani_id', '=', liquidation.resumen_plani_id.id),
													 ('fiscal_year_id', '=', liquidation.fiscal_year_id.id),
													 ('type', '=', record_type),
													 ('company_id', '=', liquidation.company_id.id)])
