...
class FunctionResultWizard(models.TransientModel):
	...
	_description = 'Function Result Wizard'
	...
	def delete_childs(self):
		...
		form_view.sudo().write({'arch': etree.tostring(xml_form)})
	...
	def insert_data(self,DynamicRecord,groups):
		...
		for group in groups:
			...
			for c,current in enumerate(currents):
				DynamicRecord.sudo().write({'x_%s_%s' % (c, group['code']): -1.0 * current.total})
				
			if group['code'] == 'F2':
				DynamicRecord.sudo().write({'x_total_%s' % group['code']: group['total']})
				DynamicRecord.sudo().write({'x_utility':total_F1 + total_F2})
			else:
				DynamicRecord.sudo().write({'x_total_%s' % group['code']: group['total']})
