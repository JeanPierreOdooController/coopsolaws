...
class ImportJournalEntryIt(models.TransientModel):
	...
	_description = 'Import Journal Entry It'
	...
	def make_move(self, values):
		...
		if move_search:
			if move_search.date.strftime('%Y-%m-%d') != values.get('date'):
				raise Warning(_('Fila %d: La Fecha Contable "%s" es diferente al elegido en la cabecera de la primera linea.\n Por favor define la misma. Asiento encontrado %s(%d)') % (values.get('row'),values.get('date'),move_search.name,move_search.id))
	...
	def make_move_line(self,values,move_id):
		...
		partner_id = None 
		if str(values.get('partner_id_aml')) != '':
			s = str(values.get("partner_id_aml"))
			vat = s.rstrip('0').rstrip('.') if '.' in s else s
			partner_id = self.find_partner(vat)

		type_document_id = None
		if str(values.get('td_aml')) != '':
