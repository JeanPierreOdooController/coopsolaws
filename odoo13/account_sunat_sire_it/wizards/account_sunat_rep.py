...
class AccountSunatRep(models.TransientModel):
	...
	nro_envio = fields.Char(string='N° de Envío',size=2)
	...
	sire_proposal = fields.Selection([('api','API'),('txt','TXT')],default='api',string='Obtener datos de')
	...
	def get_ventas_sire_1(self):
		...
		if self.type_show == 'pantalla':
			...
			return {
				'name': 'REGISTRO DE VENTAS - RVIE SUNAT %s %s'%(self.company_id.partner_id.vat,self.period.code),
			...
			worksheet.merge_range(1,0,1,8, "REGISTRO DE VENTAS - RVIE SUNAT%s %s"%(self.company_id.partner_id.vat,self.period.code), formats['especial5'] )
			...
			return self.env['popup.it'].get_file('REGISTRO DE VENTAS - RVIE SUNAT %s %s.xlsx'%(self.company_id.partner_id.vat,self.period.code),base64.encodebytes(b''.join(f.readlines())))
	...
	def get_compras_sire_1(self):
		...
		worksheet.merge_range(1,0,1,8, "REGISTRO DE COMPRAS - RCE SUNAT%s %s"%(self.company_id.partner_id.vat,self.period.code), formats['especial5'] )
		...
		return self.env['popup.it'].get_file('REGISTRO DE COMPRAS - RCE SUNAT %s %s.xlsx'%(self.company_id.partner_id.vat,self.period.code),base64.encodebytes(b''.join(f.readlines())))
	...
	def get_compras_sire_excel_3(self):
		return self.get_excel_sire(3,"c")
	...
	def get_excel_sire(self,nro,type):
		...
		for l in BOOK_SIRE:
			if l["nro"] == nro and l["type"] == type:
				name_file = l["name"]
		workbook = self.get_excel_sql_export(sql,columns)
	...
	def get_nomenclatura(self,nro,type,res):
		...
		if type == "v":
			if nro == 2:
				name_doc = "LE"+ruc+str(self.period.date_start.year)+str('{:02d}'.format(self.period.date_start.month)) + "00140400021"+valor+("1" if self.company_id.currency_id.name == 'PEN' else "2") + "2.txt"
			elif nro == 3:
				name_doc = "LE"+ruc+str(self.period.date_start.year)+str('{:02d}'.format(self.period.date_start.month)) + "00140400031"+valor+("1" if self.company_id.currency_id.name == 'PEN' else "2") +"2%s.txt"%(self.nro_envio)
			elif nro == 4:
				name_doc = "LE"+ruc+str(self.period.date_start.year)+str('{:02d}'.format(self.period.date_start.month)) + "00140400041"+valor+("1" if self.company_id.currency_id.name == 'PEN' else "2") +"2%s.txt"%(self.nro_envio)
			elif nro == 5:
				name_doc = ruc+"-CPF-"+str(self.period.date_start.year)+str('{:02d}'.format(self.period.date_start.month))+"-"+str('{:02d}'.format(self.given_numer)) + ".txt"
			else:
				raise UserError ("EN DESARROLLO")
		else:
			if nro == 2:
				if not self.comp_pr:
					name_doc = "LE"+ruc+str(self.period.date_start.year)+str('{:02d}'.format(self.period.date_start.month)) + "00080400021"+valor+("1" if self.company_id.currency_id.name == 'PEN' else "2") + "2.txt"
				else:
					name_doc = ruc+"-COMP-"+str(self.period.date_start.year)+str('{:02d}'.format(self.period.date_start.month))+"-"+str(self.given_numer)+".txt"
			elif nro == 3:
				if self.compl_type == '0': #ADICIONAR
					name_doc = ruc+"-CP-"+str(self.period.date_start.year)+str('{:02d}'.format(self.period.date_start.month))+"-"+str(self.given_numer)+".txt"
				elif self.compl_type in ('1','2'): #EXCLUIR e INCLUIR
					name_doc = ruc+"-RCEINEX-"+str(self.period.date_start.year)+str('{:02d}'.format(self.period.date_start.month))+"-"+str(self.given_numer)+".txt"
			elif nro == 4:
				name_doc = "LE"+ruc+str(self.period.date_start.year)+str('{:02d}'.format(self.period.date_start.month)) + "00080400031"+valor+("1" if self.company_id.currency_id.name == 'PEN' else "2") + "2%s.txt"%(self.nro_envio)
			elif nro == 5:
				name_doc = "LE"+ruc+str(self.period.date_start.year)+str('{:02d}'.format(self.period.date_start.month)) + "00080400041"+valor+("1" if self.company_id.currency_id.name == 'PEN' else "2") + "2%s.txt"%(self.nro_envio)
			elif nro == 6:
				name_doc = "LE"+ruc+str(self.period.date_start.year)+str('{:02d}'.format(self.period.date_start.month)) + "00080500001"+valor+("1" if self.company_id.currency_id.name == 'PEN' else "2") + "2.txt"
			elif nro == 7:
				name_doc = "LE"+ruc+str(self.period.date_start.year)+str('{:02d}'.format(self.period.date_start.month)) + "00080500031"+valor+("1" if self.company_id.currency_id.name == 'PEN' else "2") + "2%s.txt"%(self.nro_envio)
			elif nro == 8:
				name_doc = "LE"+ruc+str(self.period.date_start.year)+str('{:02d}'.format(self.period.date_start.month)) + "00080500041"+valor+("1" if self.company_id.currency_id.name == 'PEN' else "2") + "2%s.txt"%(self.nro_envio)
			else:
				raise UserError ("EN DESARROLLO")
		return name_doc
	...
	def get_sql_sire(self,nro,type):
		...
		if type == "v":
			if nro in [1,2,5]:
				sql = self.sql_sire_1_2_sale(self.period.date_start,self.period.date_end,self.company_id,nro == 5)
			elif nro in [3]:
				sql = self.sql_sire_3_sale(self.period.date_start,self.period.date_end,self.company_id)
			elif nro in [4]:
				sql = self.sql_sire_4_sale(self.period.date_start,self.period.date_end,self.company_id)
			else:
				raise UserError ("EN DESARROLLO")
		else:
			if nro in [1,2]:
				sql = self.sql_sire_2_purchase(self.period.date_start,self.period.date_end,self.company_id)
			elif nro in [3]:
				sql = self.sql_sire_3_purchase(self.period.date_start,self.period.date_end,self.company_id)
			elif nro in [4]:
				sql = self.sql_sire_4_purchase(self.period.date_start,self.period.date_end,self.company_id)
			elif nro in [5]:
				sql = self.sql_sire_5_purchase(self.period.date_start,self.period.date_end,self.company_id)
			#####REVISAR
			elif nro in [6]:
				sql = self.sql_sire_6_purchase(self.period.date_start,self.period.date_end,self.company_id)
			elif nro in [7]:
				sql = self.sql_sire_7_purchase(self.period.date_start,self.period.date_end,self.company_id)
			elif nro in [8]:
				sql = self.sql_sire_8_purchase(self.period.date_start,self.period.date_end,self.company_id)
			#####REVISAR
			else:
				raise UserError ("EN DESARROLLO")
		return sql
	...
	def sql_sire_1_2_sale(self,date_from,date_to,company_id,cpf=False):
		sql = """SELECT T.* FROM (SELECT 
				...
				NULL as campo40,
				NULL as campo41
				FROM get_ventas_1_1_sunat('{date_from}', '{date_to}', {company_id},'date') vst_v
	...
	def sql_sire_2_purchase(self,date_from,date_to,company_id):
		sql = """SELECT T.* FROM (
			...
			NULL AS campo41,
			NULL AS campo42
			from get_compras_1_1_sunat('{date_from}', '{date_to}', {company_id},'date') vst_c
	...
	def get_data_from_api_sale(self):
		...
		url = 'https://api-sire.sunat.gob.pe/v1/contribuyente/migeigv/libros/rvie/propuesta/web/propuesta/{period}/comprobantes'.format(period = self.period.code)
		headers = requests.utils.default_headers()
