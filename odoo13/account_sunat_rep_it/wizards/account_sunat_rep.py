...
class AccountSunatRep(models.TransientModel):
	...
	_description = 'Account Sunat Rep'
	...
	def get_txt_servidores(self):
		...
		return FeeRep.get_plame(1,self.period.date_start,self.period.date_end,self.company_id,self.type_date)
	
	def get_txt_recibos(self):
		FeeRep = self.env['account.fee.rep']		
		return FeeRep.get_plame(2,self.period.date_start,self.period.date_end,self.company_id,self.type_date)
	...
	def _get_sql(self,type):
		...
		if type == 7:
			...
			sql = FeeRep._get_sql(self.period.date_start,self.period.date_end,self.company_id.id,'pen',self.type_date)
		...
		if type == 8:
			...
			sql = """
					...
					' '::text as campo6,						
					a1.moneda as campo7,						
					CASE WHEN a1.td_sunat is null then '00' else a1.td_sunat end as campo8,						
					CASE						
	...
	def get_pdf_libro_diario(self):
		...
		def _get_sql_vst_diariog(self):
			sql = """
				...
				vsd.cuenta,
				aa.name as des,
				coalesce(vsd.debe,0) as debe,
				coalesce(vsd.haber,0) as haber
	...
	def get_pdf_libro_mayor(self):
		...
		def _get_sql_vst_mayor(self):
			sql = """
				...
				gmd.glosa,
				coalesce(gmd.debe,0) as debe, 
				coalesce(gmd.haber,0) as haber
	...
	def get_pdf_12_cliente(self):
		...
		def _get_sql_vst_12_cliente(self):
			sql = """
				...
				WHERE LEFT(cuenta,2) = '12' and saldo_mn <> 0
	...
	def get_pdf_13_cobrar_relacionadas(self):
		...
		def _get_sql_vst_13_cobrar_relacionadas(self):
			sql = """
				...
				saldo_mn
				FROM get_saldos_sin_cierre('%s','%s',%s)
				WHERE LEFT(cuenta,2) = '13' and saldo_mn <> 0
	...
	def get_pdf_14_cobrar_acc_personal(self):
		...
		def _get_sql_vst_14_cobrar_acc_personal(self):
			sql = """
				...
				saldo_mn
				FROM get_saldos_sin_cierre('%s','%s',%s)
				WHERE LEFT(cuenta,2) = '14' and saldo_mn <> 0
	...
	def get_pdf_16_cobrar_diversas(self):
		...
		def _get_sql_vst_16_cobrar_diversas(self):
			sql = """
				...
				saldo_mn
				FROM get_saldos_sin_cierre('%s','%s',%s)
				WHERE LEFT(cuenta,2) = '16' and saldo_mn <> 0
	...
	def get_pdf_19_cobrar_dudosa(self):
		...
		def _get_sql_vst_19_cobrar_dudosa(self):
			sql = """
				...
				saldo_mn
				FROM get_saldos_sin_cierre('%s','%s',%s)
				WHERE LEFT(cuenta,2) = '19' and saldo_mn <> 0
	...
	def get_pdf_cuenta_43(self):
		...
		def _get_sql_vst_43(self):
			sql = """
				...
				saldo_mn
				FROM get_saldos_sin_cierre('%s','%s',%s)
				WHERE LEFT(cuenta,2) = '43' and saldo_mn <> 0
	...
	def get_pdf_cuenta_44(self):
		...
		def _get_sql_vst_44(self):
			sql = """
				...
				saldo_mn
				FROM get_saldos_sin_cierre('%s','%s',%s)
				WHERE LEFT(cuenta,2) = '44' and saldo_mn <> 0
	...
	def get_pdf_cuenta_45(self):
		...
		def _get_sql_vst_45(self):
			sql = """
				...
				saldo_mn
				FROM get_saldos_sin_cierre('%s','%s',%s)
				WHERE LEFT(cuenta,2) = '45' and saldo_mn <> 0
	...
	def get_pdf_cuenta_48(self):
		...
		def _get_sql_vst_48(self):
			sql = """
				...
				saldo_mn
				FROM get_saldos_sin_cierre('%s','%s',%s)
				WHERE LEFT(cuenta,2) = '48' and saldo_mn <> 0
