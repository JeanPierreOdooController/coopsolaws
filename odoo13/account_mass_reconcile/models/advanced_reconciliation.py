...
class MassReconcileAdvancedRef(models.TransientModel):
	...
	_name = "mass.reconcile.advanced.full.it"
	_inherit = "mass.reconcile.advanced"
	_description = "Mass Reconcile Advanced Full It"
	...
	def _skip_line(move_line):
		...
		skip on some conditions. ie: ref or partner_id is empty.
		"""
		return not (move_line.get("nro_comp") and move_line.get("partner_id") and move_line.get("type_document_id"))
	...
	def _matchers(move_line):
		...
		"""
		return (
			("type_document_id", move_line["type_document_id"]),
			("partner_id", move_line["partner_id"]),
			("nro_comp", move_line["nro_comp"].lower().strip()),
		)
	...
	def _opposite_matchers(move_line):
		...
		:yield: matchers as tuple ('matcher key', value(s))
		"""
		yield ("type_document_id", move_line["type_document_id"])
		yield ("partner_id", move_line["partner_id"])
		yield ("nro_comp", (move_line["nro_comp"] or "").lower().strip())