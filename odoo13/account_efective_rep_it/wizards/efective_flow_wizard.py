...
class EfectiveFlowWizard(models.TransientModel):
	...
	def delete_childs(self):
		form_view = self.env['ir.model.data'].sudo().xmlid_to_object('account_efective_rep_it.view_dynamic_efective_flow_form')
	...
	def get_window_efective_flow(self):
		self.delete_childs()
		DynamicModel = self.env['ir.model'].sudo().search([('model','=','dynamic.efective.flow')],limit=1)
		self.env['ir.model.fields'].sudo().search([('model_id','=',DynamicModel.id),('state','=','manual')]).unlink()
		self.env['dynamic.efective.flow'].sudo().search([]).unlink()
		form_view = self.env['ir.model.data'].sudo().xmlid_to_object('account_efective_rep_it.view_dynamic_efective_flow_form')
		xml_form = etree.XML(bytes(bytearray(form_view.arch,'utf-8')))
