...
class AccountDesDetailRangeRep(models.TransientModel):
	_name = 'account.des.detail.range.rep'
	_description = 'Account Des Detail Range Rep'
	...
	def get_report(self):
		...
		sql = """
			CREATE OR REPLACE view account_des_detail_range_book as (SELECT row_number() OVER () AS id,a1.* FROM get_destinos_range('%s','%s',%d) a1)""" % (
				self.period_from_id.code,
				self.period_to_id.code,
				self.company_id.id
			)
		...
		if self.type_show == 'pantalla':
			return {
				...
				'res_model': 'account.des.detail.range.book',
	...
	def get_excel(self):
		...
		for line in self.env['account.des.detail.range.book'].search([]):
		...
		return self.env['popup.it'].get_file('Detalle_Destino.xlsx',base64.encodestring(b''.join(f.readlines())))