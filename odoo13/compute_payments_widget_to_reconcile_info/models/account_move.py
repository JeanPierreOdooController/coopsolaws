...
class AccountMove(models.Model):
	...
	invoice_has_outstanding_it = fields.Selection([('yes','Si'),('no','No')],string='Tiene pendientes', compute='_compute_payments_widget_to_reconcile_info')

	def _compute_payments_widget_to_reconcile_info(self):
		for move in self:
			move.invoice_outstanding_credits_debits_widget = json.dumps(False)
			move.invoice_has_outstanding = False
			move.invoice_has_outstanding_it = 'no'
			...
			if len(lines) != 0:
				...
				move.invoice_has_outstanding_it = 'yes'