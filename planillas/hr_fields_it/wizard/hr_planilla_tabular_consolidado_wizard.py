...
class HrPlanillaTabularSalaryConsolidadoWizard(models.TransientModel):
	_name = 'hr.planilla.tabular.salary.consolidado.wizard'
	_description = 'Hr Planilla Tabular Salary Consolidado Wizard'

	name = fields.Char()
	company_id = fields.Many2one('res.company',string=u'Compañia',required=True, default=lambda self: self.env.company,readonly=True)
	# payslip_run_consolidado_id = fields.Many2one('hr.payslip.run.consolidado', string='Periodo')
	employees_ids = fields.Many2many('hr.employee','hr_planilla_salary_consolidado_employee_rel','payslip_run_consolidado_id','employee_id',string=u'Empleados')
	...
	def default_get(self, fields):
		res = super(HrPlanillaTabularSalaryConsolidadoWizard, self).default_get(fields)
		payslip_run_id = res.get('payslip_run_consolidado_id')
		res.update({'payslip_run_consolidado_id': payslip_run_id})
		return res
	...
	def onchange_allemployees(self):
		if self.allemployees==False:
			employee_ids = []
			for employe in self.payslip_run_consolidado_id.slip_ids:
	...
	def _get_sql(self,option):
		...
		sql_payslips = "and hp.id in (%s)" % (','.join(str(i) for i in self.payslip_run_consolidado_id.slip_ids.ids))
	...
	def _get_tab_payroll_sql(self,option):
		...
		""".format(
				ids = ','.join(list(map(str, self.payslip_run_consolidado_id.slip_ids.ids))),
...
--							and(hpwd.number_of_days <> 0 or hpwd.number_of_hours <> 0)
	...
	def get_excel(self,option):
		...
		worksheet.merge_range(4, 1, 4, 8, "*** PLANILLA DE SUELDOS Y SALARIOS %s ***"  % self.payslip_run_consolidado_id.periodo_id.name or '', formats['especial5'])
		...
		return self.env['popup.it'].get_file('Planilla %s.xlsx' % self.payslip_run_consolidado_id.periodo_id.name, base64.encodebytes(b''.join(f.readlines())))


	def resize_cells(self,worksheet,widths):
		CELLS=[]
		for s in itertools.islice(self.iter_all_strings(), 200):
