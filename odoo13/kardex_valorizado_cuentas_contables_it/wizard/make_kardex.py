...
class guardado_kardex_company_line(models.Model):
	...
	esingreso = fields.Text('Ingreso')
	product_id = fields.Integer('p')
	location_id = fields.Integer('location')
	doc_type_ope = fields.Char('Doc type OPE')
	ubicacion_origen = fields.Integer('uo')
	ubicacion_destino = fields.Integer('ud')
	stock_moveid = fields.Integer('smi')
	account_invoice = fields.Char('ai')
	product_account = fields.Char('pa')
	default_code = fields.Char('dc')
	unidad = fields.Char('dc')
	mrpname = fields.Char('dc')
	ruc = fields.Char('dc')
	comapnyname = fields.Char('dc')
	cod_sunat = fields.Char('dc')
	tipoprod = fields.Char('dc')
	coduni = fields.Char('dc')
	metodo = fields.Char('dc')
	cu_entrada = fields.Float('dc')
	cu_salida = fields.Float('dc')
	period_name = fields.Char('dc')
	stock_doc = fields.Char('dc')
	origen = fields.Char('origen')
	destino= fields.Char('Destino')
	type_doc= fields.Char('Destino')
	numdoc_cuadre = fields.Char('Destino')
	doc_partner= fields.Char('Destino')
	fecha_albaran = fields.Datetime('Destino')
	pedido_compra = fields.Char('Destino')
	licitacion = fields.Char('Destino')
	doc_almac = fields.Char('Destino')
	lote  = fields.Char('Destino')
	correlativovisual= fields.Integer('Destino')
...
select 
	...
	left join (
		...
		return {
		'notify':{'title':'Guardado','message':"Se guardo " + str(registro) +" registros.<br/> Fecha Inicio: "+str(self.fini)+"<br/> Fecha Final: " + str(self.ffin),'sticky':True,'type':'success'},
		}
