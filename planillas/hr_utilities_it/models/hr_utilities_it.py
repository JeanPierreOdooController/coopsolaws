...
class HrUtilitiesIt(models.Model):
	...
	def calculate(self):
		...
		# print("res",res)
		if not res:
			raise UserError('Revisar si en las planilla consolidadas esta la regla salarial "Remuneracion Afecta Utilidades"')
		for elem in res:
		...
		self.factor_salary = ((self.distribution * 0.50) / self.sum_salary_year) if self.sum_salary_year > 0 else 0
		self.factor_number_of_days = ((self.distribution * 0.50) / self.sum_number_of_days_year) if self.sum_number_of_days_year > 0 else 0
	...
	def _get_sql_utlities(self,rule_total_income,wd_dtrab,wd_falt):
        ...
        LEFT JOIN (
			SELECT t5.employee_id,t5.dias -t6.dias as dias 
                ...
                FROM(
                    ...
                    FROM hr_resumen_planilla_line_wd hpwd
                    LEFT JOIN hr_resumen_planilla_line hp ON hp.id = hpwd.resumen_wd_line_id
                    inner join hr_resumen_planilla hrp on hrp.id = hp.resumen_plani_id
                    WHERE to_char(hrp.date_start::timestamp with time zone, 'yyyy'::text) = '{year}' AND
                        to_char(hrp.date_end::timestamp with time zone, 'yyyy'::text) = '{year}' AND 
			...
			LEFT JOIN(
			SELECT hp.employee_id,coalesce(sum(hp.holidays),0) as dias 
        ...
        WHERE to_char(hrp.date_start::timestamp with time zone, 'yyyy'::text) = '{year}'
        AND to_char(hrp.date_end::timestamp with time zone, 'yyyy'::text) = '{year}' 
