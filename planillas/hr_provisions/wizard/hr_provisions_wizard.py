	...
	def generate_move(self):
		...
		if self.debit > self.credit:
			extra_line = {
						...
						'nro_comp' : 'PROVISION'+(Provision.resumen_plani_id.periodo_id.code).replace("-", ""),
		...
		t = self.env['account.move'].create({
				...
				'date': Provision.resumen_plani_id.date_end,
				'glosa': 'PROVISION DE BBSS '+(Provision.resumen_plani_id.periodo_id.code).replace("-", ""),
				'ref': 'PROVISION'+(Provision.resumen_plani_id.periodo_id.code).replace("-", ""),
				'line_ids': [(0, 0, {
