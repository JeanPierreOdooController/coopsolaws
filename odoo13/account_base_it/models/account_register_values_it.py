...
class AccountRegisterValuesIt(models.Model):
	...
	def crear_asiento(self):
		...
		miscellaneous_journal = self.env['main.parameter'].search([('company_id','=',self.company_id.id)],limit=1).miscellaneous_journal

		if not miscellaneous_journal:
			raise UserError(u'No existe un Diario Misceláneo configurado en Parametros Generales de Contabilidad para su Compañía.')
