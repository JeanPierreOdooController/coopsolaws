...
class ProductImport(models.TransientModel):
			...
			if row_no <= 0:
				...
				if self.import_product_type == 'create':
					...
					if line[5]=='':
						...
						uom_search_id  = product_uom_obj.search([('name','=',line[5])],limit=1)
						uom_id = uom_search_id.id
					
					if line[6]=='':
						uom_po_id = 1
					else:
						uom_po_search_id  = product_uom_obj.search([('name','=',line[6])],limit=1)
	...
	def product_create(self, values):
		...
		if values.get('uom_id')=='':
			...
			uom_search_id  = product_uom_obj.search([('name','=',values.get('uom'))],limit=1)
			uom_id = uom_search_id.id
		
		if values.get('uom_po_id')=='':
			uom_po_id = 1
		else:
			uom_po_search_id  = product_uom_obj.search([('name','=',values.get('po_uom'))],limit=1)
			uom_po_id = uom_po_search_id.id
