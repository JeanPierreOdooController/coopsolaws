...
class AccountMoveReversal(models.TransientModel):
	...
	def _prepare_default_reversal(self, move):
		...
		credit_note = self.env['main.parameter'].search([('company_id', '=', self.env.company.id)],
														limit=1).dt_national_credit_note
		if not credit_note:
			raise UserError('No se definio el tipo de documento para nota de credito en parametros principales : campo : dt_national_credit_note')

		res['ref'] = self.reason if self.reason else ''
		res['type_document_id'] = credit_note.id or '',
		res['serie_id'] = None
		res['doc_invoice_relac'] = [(0, 0, {'type_document_id': move.type_document_id.id,
