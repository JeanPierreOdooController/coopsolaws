...
class AccountPurchaseRep(models.TransientModel):
	...
	_description = 'Account Purchase Rep'
	...
	def _get_sql_ple81(self,x_date_ini,x_date_end,x_company_id):
		sql = """SELECT T.* FROM (select vst_c.periodo || '00' as campo1,
			...
			where (vst_c.fecha_cont between '%s' and '%s') and vst_c.company = %d
			...
			from vst_compras_1_1 vst_c
			left join account_move am on am.id = vst_c.am_id
			where (am.date_modify_purchase between '%s' and '%s') and vst_c.company = %d and vst_c.fecha_cont < '%s'
	...
	def _get_sql_ple82(self,x_date_ini,x_date_end,x_company_id):
		sql = """SELECT T.* FROM (select 
				...
				and (vst_c.fecha_cont between '%s' and '%s') and vst_c.company = %d
				UNION ALL
				select 
				vst_c.periodo || '00' as campo1,
				vst_c.periodo || vst_c.libro || vst_c.voucher as campo2,
				...
				LEFT JOIN einvoice_catalog_01 ec01 ON ec01.id = am.campo_11_purchase_nd
				WHERE  rp1.is_not_home = TRUE
				and (am.date_modify_purchase between '%s' and '%s') and vst_c.company = %d and vst_c.fecha_cont < '%s')T
				ORDER BY T.campo1, T.campo2, T.campo3
				""" % (x_date_ini.strftime('%Y/%m/%d'),
			x_date_end.strftime('%Y/%m/%d'),
			x_company_id,
