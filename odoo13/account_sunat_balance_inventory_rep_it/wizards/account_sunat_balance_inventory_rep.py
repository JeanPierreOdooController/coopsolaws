...
class AccountSunatBalanceInventoryRep(models.TransientModel):
	...
	def _get_sql_42(self,period,company_id):
		...
		FROM get_saldos('%s','%s',%d,1) GS
		LEFT JOIN account_move_line aml ON aml.id = GS.move_line_id
		WHERE left(GS.cuenta,2) in ('42','43')
		"""% (period.code+'00',
			period.date_start.strftime('%Y/%m/%d'),
			period.date_end.strftime('%Y/%m/%d'),
			company_id)
	...
	def _get_sql_46(self,period,company_id):
		...
		FROM get_saldos('%s','%s',%d,1) GS
		LEFT JOIN account_move_line aml ON aml.id = GS.move_line_id
		WHERE left(GS.cuenta,2) in ('46','47')
		"""% (period.code+'00',
			period.date_start.strftime('%Y/%m/%d'),
			period.date_end.strftime('%Y/%m/%d'),
			company_id)
	...
	def _get_sql_20_21(self,period,company_id):
		sql = """
			...
			ei13.code as campo7,
			'1' as campo8,
			T2.saldo_fisico as campo9,
			T2.costo_prom as campo10,
			T2.saldo_valorado as campo11,
			'1' as campo12,
			NULL as campo13
			FROM
			(SELECT T.*,
	...
	def _get_ple(self,type):
		...
		che = True
		mod = self.env['ir.module.module'].search([('name', '=', 'stock')])
		sql_ple,nomenclatura = self._get_sql_nom(type)
		if type == 7 and mod.state != 'installed':
			che = False
		if che:
			...
			name_doc += nomenclatura
			name_doc += self.cc+"1"+ "0" + ("1" if mond == 'PEN' else "2") + "1.txt"
			return name_doc, base64.encodestring(b"== Sin Registros ==")
