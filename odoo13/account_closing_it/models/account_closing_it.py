...
class AccountClosingIt(models.Model):
	...
	def preview_closing(self):
		...
		return self.env['popup.it'].get_file('Preview - Cierre Contable.xlsx',base64.encodestring(b''.join(f.readlines())))
	...
	def cierre_contable_oficial(self):
		...
		if len(lines)>0:
			...
			self.account_move_id.modify_date_aml()
		else:
	...
	def cierre_contable_0(self):
		...
		if len(acc) > 0:
			for elemnt in acc:
				self.env.cr.execute("""SELECT * FROM cierre_contable_view WHERE account_id = %s and cierre_id = %d""" % (str(elemnt['account_close_id']),self.id))
	...
	def cierre_contable_3(self):
		...
		balance_sheet_account = self.env['main.parameter'].search([('company_id','=',self.company_id.id)],limit=1).balance_sheet_account
		profit_result_account = self.env['main.parameter'].search([('company_id','=',self.company_id.id)],limit=1).profit_result_account
		...
		lost_sheet_account = self.env['main.parameter'].search([('company_id','=',self.company_id.id)],limit=1).lost_sheet_account
		lost_result_account = self.env['main.parameter'].search([('company_id','=',self.company_id.id)],limit=1).lost_result_account
