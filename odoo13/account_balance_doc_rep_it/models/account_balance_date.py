...
class AccountBalanceDateBook(models.Model):
	_name = 'account.balance.date.book'
	_description = 'Account Balance Date Book'
	_auto = False
	...
	def init(self):
		...
		self.env.cr.execute('''
			CREATE OR REPLACE VIEW %s AS (
				SELECT a1.* FROM get_saldos('2019/01/01','2019/01/01',1,3) a1 limit 1
