...
class AccountMove(models.Model):
    ...
    is_detraccion_anticipo = fields.Boolean(string="Es una factura con Anticipo y Detracción")
        ...
        for record in self:
            ...
            if parameter_ebill.detraccion_in_observaciones:
                if record.retencion_amount and record.detraction_amount:
                    ...
                    retencion = round(record.retencion_amount, 2)

                    if parameter_ebill.rounded_detraccion:
                        if record.retencion_amount >= parameter_ebill.rounded_detraccion_from :
                            retencion = round(record.retencion_amount)



                    narration = txt_monto + str(retencion) + "\n"
