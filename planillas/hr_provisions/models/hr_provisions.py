...
class HrProvisiones(models.Model):
	...
	resumen_plani_id = fields.Many2one('hr.resumen.planilla', string='Planilla')
	...
	def actualizar(self):
		...
		date_start = date_utils.subtract(self.resumen_plani_id.date_start, months=5)
		date_end = date_utils.subtract(self.resumen_plani_id.date_end, days=0)
		...
		for employee in employees:
			sql = """
				...
				sum(hpl.amount) as total,
				max(hsi.percent) as porcentaje
				FROM hr_resumen_planilla_line_salary hpl
				LEFT JOIN hr_resumen_planilla_line hp ON hp.id = hpl.resumen_salary_line_id
				inner join hr_resumen_planilla hrp on hrp.id = hp.resumen_plani_id
				...
				and hrp.date_start = '%s'
				and hrp.date_end = '%s'
				and hc.labor_regime in ('general','small')
				and hpl.code = '%s'
--				and resumen_plani_id is not null
				group by hp.employee_id
			"""%(employee.id, self.resumen_plani_id.date_start, self.resumen_plani_id.date_end, MainParameter.basic_sr_id.code)
			self._cr.execute(sql)
			data = self._cr.dictfetchall()

			payslips = self.env['hr.resumen.planilla.line'].search([('employee_id','=',employee.id),
													   ('resumen_plani_id.date_start','>=',date_start),
													   ('resumen_plani_id.date_end','<=',date_end),
													   ('resumen_plani_id','!=',None)])
			...
			for l in payslips:
				# print("l",l)
				for k in l.slip_salary_ids:
					# print("k",k)
					if k.salary_rule_id.id in comi_ids:
						# print("total",k.total)
						amount_comisi = amount_comisi + k.amount
						ncomis=ncomis+1

					if k.salary_rule_id.id in boni_ids:
						amount_bonifi = amount_bonifi + k.amount
						nboni=nboni+1

					if k.salary_rule_id.id == extrhors_ids:
						amount_hextra = amount_hextra + k.amount
						nextra=nextra+1
			# print("n comisiones",ncomis)
			for payslip in data:
				Contract = self.env['hr.contract'].browse(payslip['contract_id'])
				if Contract.situation_id.name == 'BAJA':
					if self.resumen_plani_id.date_start <= Contract.date_end <= self.resumen_plani_id.date_end:
						continue
				...
				if Contract.date_start <= self.resumen_plani_id.date_start:
			...
			sql_provision = """
							...
							from (
								select 
									...
									inner join hr_resumen_planilla hpr on hpr.id = hpro.resumen_plani_id
				...
				resumen_plani_id=self.resumen_plani_id.id,
				provision_id=self.id)
		else:
			sql_provision = """
							union all
	...
	def get_provisiones_excel(self):
		...
		worksheet.write(3,1,self.resumen_plani_id.periodo_id.name,formats['especial1'])
		...
		return self.env['popup.it'].get_file('Provisiones - %s.xlsx' % self.resumen_plani_id.periodo_id.name,base64.encodebytes(b''.join(f.readlines())))
...
class HrProvisionesCtsLine(models.Model):
	...
	def _get_prov_cts(self):
		for record in self:
			...
			if record.fecha_ingreso > record.provision_id.resumen_plani_id.date_start and record.fecha_ingreso <= record.provision_id.resumen_plani_id.date_end:
