...
class AccountPeriod(models.Model):
	...
	close = fields.Boolean(string='Cerrado', default=False,tracking=True)
	is_opening_close = fields.Boolean(string=u'Apertura/Cierre',default=False,tracking=True)
	...
	def _verify_code(selfs):
		for self in selfs:
			if len(self.env['account.period'].search([('code','=',self.code)],limit=2)) > 1:
