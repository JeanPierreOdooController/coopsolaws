	...
	def _get_period(self):
		for record in self:
			if record.type and record.fiscal_year_id.name:
				...
				Period = self.env['hr.resumen.planilla'].search([('date_start', '=', date_start),
															('date_end', '=', date_end)], limit=1)
				if Period:
					record.resumen_plani_id = Period.id
...
class HrCtsLine(models.Model):
	...
	def send_cts_by_email(self):
			...
			try:
				self.env['mail.mail'].create({
					...
					'body_html': 'Estimado (a) %s,<br/>'
								 ...
								 '<strong>Nota: Para abrir su certificado es necesario colocar su dni como clave</strong>' % (
								 Employee.name, dict(payslip.cts_id._fields['type'].selection).get(payslip.cts_id.type),
								 payslip.cts_id.fiscal_year_id.name),
		...
		if issues:
			return self.env['popup.it'].get_message(
				'No se pudieron enviar los certificados de los siguientes Empleados: \n %s' % '\n'.join(issues))
		else:
	...
	def get_vouchers_cts(self, payslips=None):
		...
		if not MainParameter.employee_in_charge_id:
			raise UserError(
				'Falta configurar un Encargado para Liquidacion Semestral en Parametros Principales en la Pestaña de CTS')
		route = MainParameter.dir_create_file
		doc = SimpleDocTemplate(route + "LIQUIDACION DE DEPOSITO SEMESTRAL CTS.pdf", pagesize=A4, topMargin=20,
								bottomMargin=20)
		elements = []
		...
		return self.env['popup.it'].get_file('LIQUIDACION DE DEPOSITO SEMESTRAL CTS.pdf',
	...
	def get_pdf_cts(self):
		...
		if self.cts_id.type == '11':
			...
			period = 'Del 01 de Noviembre {last_year} del al 30 de Abril del {year}'.format(last_year=year - 1,
																							year=year)
		period += u': {months} meses, {days} días'.format(months=self.months, days=self.days)
		datat = [
			[Paragraph('<b>1. <u>FECHA DE INGRESO</u>: </b> %s' % self.admission_date.strftime('%d/%m/%Y'),
					   styles["Left"]), '', ''],
			...
			[Paragraph('-  Asignación Familiar', styles["Tab"]), 'S/.',
			 Paragraph(str(self.household_allowance), styles["Right"])],
			[Paragraph('-  Horas Extra', styles["Tab"]), 'S/.', Paragraph(str(self.extra_hours), styles["Right"])],
			[Paragraph('-  Gratificaciones', styles["Tab"]), 'S/.',
			 Paragraph(str(self.sixth_of_gratification), styles["Right"])],
		...
		datat += [[Paragraph('TOTAL', styles["RightBold"]), 'S/.',
				   Paragraph(str(self.computable_remuneration), styles["RightBold"])],
				  [Paragraph('<u>CALCULO</u>', styles["LeftBold"]), '', ''],
				  [Paragraph('  -  Por los meses completos:', styles["LeftBold"]), '', ''],
				  [Paragraph("S/. {0} ÷ 12 x {1} mes(es)".format(self.computable_remuneration, self.months),
							 styles["Tab"]),
				   'S/.', Paragraph(str(self.cts_per_month), styles["Right"])],
				  [Paragraph('  -  Por los dias completos:', styles["LeftBold"]), '', ''],
				  [Paragraph("S/. {0} ÷ 12 ÷ 30 x {1} día(s)".format(self.computable_remuneration, self.days),
							 styles["Tab"]),
				   'S/.', Paragraph(str(self.cts_per_day), styles["Right"])],
				  [Paragraph('  -  Faltas:', styles["LeftBold"]), '', ''],
				  [Paragraph("S/. {0} ÷ 12 ÷ 30 x {1} día(s)".format(self.computable_remuneration, self.lacks),
							 styles["Tab"]),
				   'S/. ', Paragraph(str(-self.amount_per_lack), styles["Right"])],
				  [Paragraph('  -  Interes CTS:', styles["LeftBold"]), 'S/. ',
				   Paragraph(str(self.cts_interest), styles["Right"])],
				  [Paragraph('  -  Otros Descuentos:', styles["LeftBold"]), 'S/. ',
				   Paragraph(str(-self.other_discounts), styles["Right"])],
		...
		if cts_account.currency_id.name == 'USD':
			datat = [
				[Paragraph('MONTO DEPOSITADO (*)', styles["LeftBold"]), '$',
				 Paragraph(str(self.cts_dollars), styles["RightBold"])],
				['', '', ''],
