...
CREATE OR REPLACE VIEW public.payslip_run_move
	...
	inner join hr_resumen_planilla_line hp on hp.id = hpl.resumen_salary_line_id
...
RETURN QUERY
	...
	round(sum(prm.amount)::numeric, 2) as debit,
	0::numeric as credit,
	null::integer as partner_id
	from payslip_run_move prm
	where prm.resumen_plani_id = $1 and
	...
	null::integer as partner_id
	from payslip_run_move prm
	inner join hr_resumen_planilla_line hp on hp.id = prm.slip_id
...
--    aaa.id as analytic_account_id,
    prm.account_debit as account_id,
    CASE WHEN aa.check_moorage THEN round(sum(prm.amount * hadl.percent * 0.01)::numeric, 2) ELSE round(sum(prm.amount)::numeric, 2) END AS debit,
    0::numeric as credit,
    null::integer as partner_id
    from payslip_run_move prm
    inner join hr_resumen_planilla_line hp on hp.id = prm.slip_id
    ...
    from (
        ...
        prm.account_credit as account_id,
        0::numeric as debit,
        CASE WHEN aa.check_moorage THEN round((prm.amount * hadl.percent * 0.01)::numeric, 2) ELSE round(prm.amount::numeric, 2) END AS credit,
        null::integer as partner_id
        from payslip_run_move prm
        inner join hr_salary_rule hsr on hsr.id = prm.salary_rule_id
        inner join hr_resumen_planilla_line hp on hp.id = prm.slip_id
        ...
        where hsr.code not in ('COMFI','COMMIX','SEGI','A_JUB','NETREMU','EPS','NETVACA','CTS','CTS_TRU','VATRU','GRA','BON9','GRA_TRU','BON9_TRU','NETO') and
        prm.resumen_plani_id = $1 and
        prm.company_id = $2 and
        prm.account_credit is not null
        ...
        prm.account_credit as account_id,
        0::numeric as debit,
        case when hsr.code in ('NETREMU','EPS','NETVACA','CTS','CTS_TRU','VATRU','GRA','BON9','GRA_TRU','BON9_TRU','NETO') then prm.amount else 0 end credit,
        ...
        where hsr.code in ('NETREMU','EPS','NETVACA','CTS','CTS_TRU','VATRU','GRA','BON9','GRA_TRU','BON9_TRU','NETO') and
        prm.resumen_plani_id = $1 and
        prm.company_id = $2 and
        prm.account_credit is not null
