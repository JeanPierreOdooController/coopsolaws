...
class HrLiquidation(models.Model):
    ...
    resumen_plani_id = fields.Many2one('hr.resumen.planilla', string='Periodo', required=True, states={'exported': [('readonly', True)]})
    ...
    state = fields.Selection([('draft', 'Borrador'), ('exported', 'Hecho')], default='draft', string='Estado')
    ...
    @api.onchange('fiscal_year_id', 'resumen_plani_id')
    def _get_name(self):
        for record in self:
            if record.fiscal_year_id and record.resumen_plani_id:
                record.name = 'Liquidacion {0}'.format(record.resumen_plani_id.periodo_id.name)

    @api.onchange('resumen_plani_id')
    def _get_type(self):
        for record in self:
            if record.resumen_plani_id:
                month = record.resumen_plani_id.date_start.month
                if record.resumen_plani_id and month > 6:
                    record.gratification_type = '12'
                else:
                    record.gratification_type = '07'
                if record.resumen_plani_id and ((10 < month and month <= 12) or (1 <= month and month < 5)):
                    record.cts_type = '05'
                else:
                    record.cts_type = '11'
    ...
    def get_vacation_lines(self):
        ...
        MonthLot = self.resumen_plani_id
        Employees = MonthLot.slip_ids.filtered(lambda slip: slip.contract_id.labor_regime in ['general', 'small','micro'] and
                                                            ...
                                                            slip.periodo_id.date_start <= slip.contract_id.date_end and
                                                            slip.periodo_id.date_end >= slip.contract_id.date_end and
                                                            slip.contract_id.partner_id.id == self.partner_id.id and
                                                            not slip.contract_id.less_than_four).mapped('employee_id')

        for Employee in Employees:
            # print('empleados',Employee.name)
            bonus_months = months = days = lacks = 0
            Commissions = Bonus = ExtraHours = self.env['hr.resumen.planilla.line.salary']
            ...
            # print('wage',wage)
            household_allowance = MonthSlip.slip_salary_ids.filtered(lambda line: line.salary_rule_id == MainParameter.household_allowance_sr_id).amount
            ...
            Lots = self.env['hr.resumen.planilla'].search([('date_start', '>=', compute_payslip_date), ('date_end', '<=', MonthSlip.periodo_id.date_end)])
            # print('Lots primero',Lots)
            for Lot in Lots:
                # print('Lot',Lot.name)
                EmployeeSlips = Lot.slip_ids.filtered(lambda slip: slip.employee_id == Employee)
                SalaryRules = EmployeeSlips.mapped('slip_salary_ids')
                WorkedDays = EmployeeSlips.mapped('slip_wd_ids')
                WorkingWD =	sum(WorkedDays.filtered(lambda line: line.wd_type_id in MainParameter.working_wd_ids).mapped('number_of_days'))
                if Lot.date_end.month == 2 and WorkingWD >= 28:
                    months += 1
                elif WorkingWD >= 30:
                    months += 1
                else:
            ...
            Lots = self.env['hr.resumen.planilla'].search([('date_start', '>=', date_limit_from), ('date_end', '<=', date_limit_to)])
            # print('Lots',Lots)
            ...
            for Lot in Lots:
                ...
                EmployeeSlips = Lot.slip_ids.filtered(lambda slip: slip.employee_id == Employee)
                SalaryRules = EmployeeSlips.mapped('slip_salary_ids')
                Commissions += SalaryRules.filtered(lambda line: line.salary_rule_id in MainParameter.commission_sr_ids and line.amount > 0)
                Bonus += SalaryRules.filtered(lambda line: line.salary_rule_id in MainParameter.bonus_sr_ids and line.amount > 0)
                ExtraHours += SalaryRules.filtered(lambda line: line.salary_rule_id == MainParameter.extra_hours_sr_id and line.amount > 0)
            # 	months += 1
            # print("month",months)
    ...
    def get_liquidation(self):
        ...
        preservados = self.env['hr.liquidation.vacation.line'].search([('liquidation_id', '=', self.id), ('preserve_record', '=', True)])
        empleados_pre = []
        for j in preservados:
            if j.employee_id.id not in empleados_pre:
                empleados_pre.append(j.employee_id.id)
    ...
    def get_excel_liquidation(self):
        ...
        doc_name = '%s %s.xlsx' % ('Liquidacion', self.resumen_plani_id.periodo_id.name)
...
class HrExtraConceptLine(models.Model):
    ...
    is_afiliacion = fields.Boolean(string='Afecto AFP/ONP', default=False)
    amount_afiliacion = fields.Float(string='(-) AFP/ONP', compute='get_afiliacion', store=True)
    neto_amount = fields.Float(string='Neto Total', compute='get_afiliacion', store=True)

    @api.depends('amount','is_afiliacion')
    def get_afiliacion(self):
        ReportBase = self.env['report.base']
        for record in self:
            if record.extra_concept_id.concept_id.contract_id and record.is_afiliacion:
                membership = record.extra_concept_id.concept_id.contract_id.membership_id
                onp = afp_jub = afp_si = afp_mixed_com= afp_fixed_com= 0
                if membership.is_afp:
                    afp_jub = ReportBase.custom_round(membership.retirement_fund/100 * record.amount, 2)
                    if record.amount>=membership.insurable_remuneration:
                        afp_si = ReportBase.custom_round(membership.prima_insurance/100 * membership.insurable_remuneration, 2)
                    else:
                        afp_si = ReportBase.custom_round(membership.prima_insurance/100 * record.amount, 2)
                    if record.extra_concept_id.concept_id.contract_id.commision_type == 'mixed':
                        afp_mixed_com = ReportBase.custom_round(membership.mixed_commision/100 * record.amount, 2)
                        afp_fixed_com =0
                    elif record.extra_concept_id.concept_id.contract_id.commision_type == 'flow':
                        afp_fixed_com = ReportBase.custom_round(membership.fixed_commision /100 * record.amount, 2)
                        afp_mixed_com =0
                else:
                    onp = ReportBase.custom_round(membership.retirement_fund/100 * record.amount, 2)
                record.amount_afiliacion = ReportBase.custom_round(afp_jub - afp_si - afp_mixed_com - afp_fixed_com - onp, 2)
                # self.neto_amount = ReportBase.custom_round(self.amount-self.amount_afiliacion, 2)
            else:
                record.amount_afiliacion=0
            record.neto_amount = ReportBase.custom_round(record.amount-record.amount_afiliacion, 2)