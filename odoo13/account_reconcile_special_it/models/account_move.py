...
class AccountMove(models.Model):
	...
	def action_concile_special(self):
		for move in self:
			...
			if apply:
				self.env.cr.execute("""update account_move set amount_residual = 0, amount_residual_signed = 0, invoice_payment_state='paid' where id = %d"""%(move.id))
	...
	def action_reconcile_special(self):
		for move in self:
			self.env.cr.execute("""update account_move set amount_residual = amount_total, amount_residual_signed = amount_total_signed, invoice_payment_state='not_paid' where id = %d"""%(move.id))
	...
	def modify_date_aml(self):
		for move in self:
			self.env.cr.execute("""update account_move_line set date = null where move_id = %d"""%(move.id))

class AccountMoveLine(models.Model):
	_inherit = "account.move.line"

	def action_remove_reconcile_special(self):
		for line in self:
			if line.amount_residual == 0 and line.reconciled and not line.full_reconcile_id:
				self.env.cr.execute("""update account_move_line set reconciled=FALSE, amount_residual = (debit - credit) , amount_residual_currency = amount_currency where id = %d"""%(line.id))