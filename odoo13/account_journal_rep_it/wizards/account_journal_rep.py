...
class AccountJournalRep(models.TransientModel):
	...
	_description = 'Account Journal Rep'
	...
	def _get_sql_ple(self,type,x_date_ini,x_date_end,x_company_id):
		...
		if type == 1 or type == 2:
			...
			where (am.date between '%s' and '%s') and am.company_id = %d  and (aml.cuo is null or aml.cuo = 0)""" % (x_date_ini.strftime('%Y/%m/%d'),x_date_end.strftime('%Y/%m/%d'),x_company_id))
			...
			sql = """ SELECT T.* FROM (SELECT 
					...
					(CASE
						...
						WHEN vst_d.nro_comprobante is not null and position('-' in vst_d.nro_comprobante::text) <> 0 AND split_part(vst_d.nro_comprobante, '-', 2) <> '' THEN replace(replace(split_part(vst_d.nro_comprobante, '-', 2),'_',''),':','')
						WHEN vst_d.nro_comprobante is not null and position('-' in vst_d.nro_comprobante::text) = 0 AND split_part(vst_d.nro_comprobante, '-', 1) <> '' THEN replace(replace(split_part(vst_d.nro_comprobante, '-', 1),'_',''),':','')
						ELSE 'SN'
					END) ELSE (CASE WHEN coalesce(vst_d.nro_comprobante,'') <> '' THEN replace(TRIM(LEFT(vst_d.nro_comprobante,20)),':','') ELSE 'SN' END) END AS campo12,
					...
					WHEN vst_d.nro_comprobante is not null and position('-' in vst_d.nro_comprobante::text) = 0 AND split_part(vst_d.nro_comprobante, '-', 1) <> '' THEN replace(replace(split_part(vst_d.nro_comprobante, '-', 1),'_',''),':','') 
					ELSE 'SN' END)
					ELSE (CASE WHEN coalesce(vst_d.nro_comprobante,'') <> '' THEN replace(TRIM(LEFT(vst_d.nro_comprobante,20)),':','') ELSE 'SN' END) END AS campo12,
					TO_CHAR(vst_d.fecha::DATE, 'dd/mm/yyyy') AS campo13,
					...
					vst_d.medio_pago AS campo24, 
					' ' AS campo25
					FROM get_correccion_diariog('%s','%s',%d) vst_d
					LEFT JOIN account_move_line aml ON aml.id = vst_d.move_line_id
					LEFT JOIN account_move am ON am.id = vst_d.move_id
					LEFT JOIN account_journal aj ON aj.id = am.journal_id
					LEFT JOIN res_partner rp ON rp.id = vst_d.partner_id
					LEFT JOIN account_account aa on aa.id = vst_d.account_id
					WHERE aml.cuo is not null)T
					%s
