...
class HrPayslipRunMoveWizard(models.TransientModel):
	...
	name = fields.Char()
	# payslip_run_consolidado_id = fields.Many2one('hr.payslip.run.consolidado', string='Planilla')
	...
	def generate_move(self):
		...
		# print("lines",lines)
		PR = self.env['hr.resumen.planilla'].browse(self._context.get('resumen_plani_id'))
		extra_line = [(0, 0, extra_line)] if extra_line else []
		move = self.env['account.move'].create({
			'journal_id': self.journal_id.id,
			'date': PR.periodo_id.date_end,
			'ref': 'PLANILLA'+(PR.periodo_id.code).replace("-", ""),
			'line_ids': extra_line + [
				(0, 0, {
					...
					'nro_comp' : 'PLANILLA'+(PR.periodo_id.code).replace("-", ""),
	...
	def get_sql(self):
		...
		sql = """
			...
			""" % (function, self.resumen_plani_id.id, self.env.company.id)
		return sql
	...
	def get_payslip_move_analysis_excel(self):
		...
		return self.env['popup.it'].get_file('Analisis_Asiento_Planilla.xlsx', base64.encodestring(b''.join(f.readlines())))