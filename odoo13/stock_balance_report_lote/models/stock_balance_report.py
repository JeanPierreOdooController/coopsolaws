...
class stock_balance_report_lote_wizard(models.TransientModel):
	_name = 'stock.balance.report.lote.wizard'
	_description = "stock balance report lote wizard"

	def get_stock_lote(self):
		return self.env['stock.balance.report.lote'].with_context({'res_model_it':'stock.balance.report.lote.wizard','id_it':self.id}).get_balance_view()
...
class StockBalanceReportLote(models.Model):
	...
	def get_balance_view(self):
		...
		self.env.cr.execute("""
			...
			left join (
				select stock_move_line.location_id,sum(stock_move_line.product_uom_qty) as reservado, stock_move_line.product_id,stock_move_line.lot_id  from stock_move_line inner join stock_production_lot  on stock_production_lot.id = stock_move_line.lot_id where stock_move_line.state in ('partially_available','assigned') 
				group by stock_move_line.location_id, stock_move_line.product_id,stock_move_line.lot_id
			) reservado on reservado.lot_id = vstf.lote_id and reservado.product_id = vstf.p_id and reservado.location_id = vstf.alm_id
			where 
		...
		for item in todos:
			...
			self.env['stock.balance.report.lote'].create(data)
