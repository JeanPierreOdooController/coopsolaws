...
class UploadChartAccountIt(models.TransientModel):
	...
	_description = 'Upload Chart Account It'
	...
	def upload_chart_parameter(self):
		obj_parameter = self.env['main.parameter']
		...
		self.env.cr.execute("""select * from update_main_parameter(%s,'%s',%s)""" % (str(self.env.company.id),self.direct_file,str(main_parameter.id)))

	def upload_chart_journal(self):
		obj = self.env['account.journal']
		obj_acc = self.env['account.account']

		rmn = obj.search([('code','=','RMN'),('company_id','=',self.env.company.id)],limit=1)
		if not rmn:
			rmn = obj.create({
				'name':'ENTREGAS A RENDIR MN',
				'code': 'RMN',
				'type':'cash',
				'company_id': self.env.company.id
			})
		rmn.check_surrender = True
		acc_rendir_mn = obj_acc.search([('code','=','1413001'),('company_id','=',self.env.company.id)],limit=1) 
		rmn.default_debit_account_id = acc_rendir_mn.id if acc_rendir_mn else None
		rmn.default_credit_account_id = acc_rendir_mn.id if acc_rendir_mn else None

		rme = obj.search([('code','=','RME'),('company_id','=',self.env.company.id)],limit=1)
		if not rme:
			rme = obj.create({
				'name':'ENTREGAS A RENDIR ME',
				'code': 'RME',
				'type': 'cash',
				'company_id': self.env.company.id,
				'currency_id': self.env['res.currency'].search([('name','=','USD')],limit=1).id
			})
		rme.check_surrender = True
		acc_rendir_me = obj_acc.search([('code','=','1413002'),('company_id','=',self.env.company.id)],limit=1) 
		rme.default_debit_account_id = acc_rendir_me.id if acc_rendir_me else None
		rme.default_credit_account_id = acc_rendir_me.id if acc_rendir_me else None
		
		aut = obj.search([('code','=','AUT'),('company_id','=',self.env.company.id)],limit=1)
		if not aut:
			aut = obj.create({
				'name':'ASIENTOS AUTOMATICOS',
				'code': 'AUT',
				'type': 'general',
				'company_id': self.env.company.id
			})
		...
		if not pdet:
			pdet = obj.create({
				'name':'PROVISION DE DETRACCIONES',
				'code': 'PDET',
				'type': 'general',
				'company_id': self.env.company.id
			})
		...
		if not anc:
			anc = obj.create({
				'name':'APLICACION DE NOTAS DE CREDITO',
				'code': 'ANC',
				'type': 'general',
				'company_id': self.env.company.id
			})
