	...
	def find_country(self,val):
		if type(val) == dict:
			...
			if country_search:
				...
				raise Warning(u'El país %s no está disponible en el sistema.'%(val.get('country')))
		else:
			country_search = self.env['res.country'].search([('name','=',val[8])])
			if country_search:
				return country_search.id
			else:
				raise Warning(u'El país %s no está disponible en el sistema.'%(val[8]))
	...
	def find_state(self,val):
		if type(val) == dict:
			...
			if state_search:
				...
				raise Warning(u'El estado %s no está disponible en el sistema.'%(val.get('state')))
		else:
			state_search = self.env['res.country.state'].search([('name','=',val[6])])
			if state_search:
				return state_search.id
			else:
				raise Warning(u'El estado %s no está disponible en el sistema.'%(val[6]))
	...
	def create_partner(self, values):
		...
		if values.get('type_document_id'):
			...
			if not l10n_latam_search:
				raise Warning("Tipo de Doc. no disponible en el sistema")
			else:
		...
		if values.get('saleperson'):
			...
			if not saleperson_search:
				raise Warning("Usuario no disponible en el sistema")
			else:
				...
				if self.partner_option == 'create':
					...
					if line[1] == 'company':
						...
						if line[2]:
							...
							if parent_search:
								...
								raise Warning("Contacto padre con Nro de Documento '%s' no disponible"%(line[2]))
					...
					if line[20]:
						...
						if not cust_payment_term_search:
							raise Warning(u"Término de pago no disponible en el sistema")
						else:
							cust_pmt_term = cust_payment_term_search.id
					if line[21]:
