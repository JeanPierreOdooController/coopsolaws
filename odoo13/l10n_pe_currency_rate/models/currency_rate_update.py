...
class PeCurrencyRateUpdateService(models.Model):
	"""Class keep services and currencies that
	have to be updated"""
	_name = "pe.currency.rate.update.service"
	...
	currency_list_readonly = fields.Many2many(
		related='currency_list', readonly=True, string='Currencies available Related')
	...
	#			curr._action_sunat_exchange_rate(self.company_id.id)

	#def refresh_currency(self):
	#	"""Refresh the currencies rates !!for all companies now"""
	#	date = self.env.context.get('date', None)
	...
	#		# The two case
	#		if company.pe_auto_currency_up:
	...
	#					date
	#					)
	#				# rate_name = date or fields.Date.context_today(self)
	#				rate_name = date or fields.Datetime.context_timestamp(self.with_context(tz=self.env.user.tz), datetime.now())
	#				for curr in srv.currency_to_update:
	...
	#						('name', '=', rate_name)])
	#					if not rates:
	#						rate = res[curr.name].get('sale_value', 0)
	#						sale_type = res[curr.name].get('sale_value', 0)
	#						purchase_type = res[curr.name].get('purchase_value', 0)
	#						
	#						vals = {
	#							'currency_id': curr.id,
	#							'rate': 1/rate,
	...
	#					log_info or '',
	#					fields.Datetime.context_timestamp(self, datetime.utcnow()),
	#					note
	#				)
	#				srv.write({'note': msg})
