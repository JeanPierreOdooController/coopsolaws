...
class AccountMove(models.Model):
	...
	def write(self, vals):
		res = True
		for move in self:
			if 'access_token' in vals or 'amount_untaxed' in vals or 'amount_tax' in vals or 'amount_total' in vals or 'amount_residual' in vals or 'amount_untaxed_signed' in vals or 'amount_tax_signed' in vals or 'amount_total_signed' in vals or 'amount_residual_signed' in vals or 'invoice_payment_state' in vals or 'release_to_pay_manual' in vals:
	...
	def create(self, vals_list):
		...
		rslt._check_journal_period_close(vals_list)
		return rslt
...
class AccountMoveLine(models.Model):
	...
	def create(self, vals_list):
		...
		moves._check_journal_period_close(vals_list)
		return res

	def write(self, vals):
		result = True
		for line in self:
			if 'full_reconcile_id' in vals or 'reconciled' in vals or 'amount_residual' in vals or 'amount_residual_currency' in vals or 'followup_line_id' in vals:
