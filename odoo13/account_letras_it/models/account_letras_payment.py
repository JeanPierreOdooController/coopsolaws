...
class AccountLetrasPayment(models.Model):
	...
	_description = 'Account Letras Payment'	
	...
	def on_change_date_exchange(self):
		if self.date_exchange:
			divisa_line = self.env['res.currency.rate'].search([('name','=',self.date_exchange)],limit=1)
			if divisa_line:
				self.tipo_cambio = divisa_line.sale_type
	...
	def crear_asiento(self):
		...
		move_id = self.env['account.move'].create({
			...
			'invoice_date': self.date_exchange,
			'line_ids':lineas,
			'ref': self.name,
			'glosa':self.glosa,
			'type':'entry'})
...
class AccountLetrasPaymentFactura(models.Model):
	...
	_description = 'Account Letras Payment Factura'	
...
class AccountLetrasPaymentManual(models.Model):
	...
	_description = 'Account Letras Payment Manual'	
	...
	def get_account(self):
		...
		if self.letra_payment_id.type == 'out' or self.env.context.get('default_type') == 'out':
			if self.account_id.id in (param.customer_letter_account_fc.id,param.customer_letter_account_nc.id) or not self.account_id:
		...
		if self.letra_payment_id.type == 'in' or self.env.context.get('default_type') == 'in':
			if self.account_id.id in (param.supplier_letter_account_fc.id,param.supplier_letter_account_nc.id) or not self.account_id:
...
class AccountLetrasPaymentResidual(models.Model):
	...
	_description = 'Account Letras Payment Residual'	
