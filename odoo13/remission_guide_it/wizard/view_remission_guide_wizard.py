	...
	def post_request(self):
		...
		for l in self.line_ids:
			if l.move_id.state == 'done':
				desc=l.product_id.name_get()[0][1].replace(l.default_code,"").replace("[]","") if l.default_code else l.product_id.name_get()[0][1]
				if parameters.use_description:
					if l.move_id.description_picking:
						desc=l.move_id.description_picking
				g_items.append({
