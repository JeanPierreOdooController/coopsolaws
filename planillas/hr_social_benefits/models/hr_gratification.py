...
class HrGratification(models.Model):
	...
	def get_gratification(self):
		...
		preservados = self.env['hr.gratification.line'].search([('gratification_id', '=', self.id), ('preserve_record', '=', True)])
		empleados_pre = []
...
class HrGratificationLine(models.Model):
	...
	def get_pdf_grati(self):
		...
		data = [
			[Paragraph('%s %d' % (
				dict(self.gratification_id._fields['type'].selection).get(self.gratification_id.type) or '',
				year or ''),
					   style_left_title),
			 Paragraph('Fecha de Pago: %s' % datetime.strftime((self.gratification_id.deposit_date), '%d-%m-%Y') or '',
					   style_cell)],
		]
		...
		if Contract.situation_id.name == 'BAJA':
			if self.gratification_id.resumen_plani_id.date_start <= Contract.date_end <= self.gratification_id.resumen_plani_id.date_end:
				situacion = 'BAJA'
			else:
		...
		third_row = [
			...
			[Paragraph('%s' % ('01/01/%d al 30/06/%d' % (
				year, year) if self.gratification_id.type == '07' else '01/07/%d al 31/12/%d' % (year, year)),
					   style_cell),
			 '',
