...
class HrAutomateMultipayment(models.Model):
    ...
    payslip_run_id = fields.Many2one('hr.payslip.run', domain=[('txt_generated', '=', False)], string='Periodo')
    cts_id = fields.Many2one('hr.cts', string='CTS', domain=[('txt_generated', '=', False)])
    ...
    def get_slip_ids(self):
        for rec in self:
            if rec.payslip_run_id and rec.payslip_run_id.slip_ids and rec.journal_id:
                Slips = rec.payslip_run_id.slip_ids.filtered(lambda l: l.wage_bank.format_bank == rec.format_bank and l.net_to_pay !=0)
    ...
    def get_cts_line_ids(self):
        for rec in self:
            if rec.cts_id and rec.cts_id.line_ids and rec.journal_id:
                cts_lines = rec.cts_id.line_ids.filtered(lambda l: l.cts_bank.id == rec.journal_id.bank_id.id and l.cts_id.company_id.id == rec.journal_id.company_id.id and not l.less_than_one_month)
    ...
    def parse_to_txt(self, var, type, range=1, left=True, decimal_point=False):
        '''Parse the type of data int, str, float, date or datetime to txt format
    ...
    def get_amounts_total_hr(self, gratification):
        ...
        Lines = Gratification.line_ids.filtered(lambda l: l.employee_id in self.slip_ids.mapped('employee_id'))
        if gratification:
            ...
            for slip in self.slip_ids:
                Gratification = self.env['hr.gratification'].search([('payslip_run_id', '=', self.payslip_run_id.id)])
                Line = Gratification.line_ids.filtered(lambda l: l.employee_id == slip.employee_id)
...
{reference}{alert_indicator}{alert_val}'.format(
        ...
        f.close()
        f = open(doc_name, 'rb')
        return self.env['popup.it'].get_file('BBVA_Haberes.txt', base64.encodestring(b''.join(f.readlines())))
...
{beneficiary_ref}{company_ref}{currency}{amount}{flag}\r\n'.format(
        ...
        f.close()
        f = open(doc_name, 'rb')
        return self.env['popup.it'].get_file('BCP_Haberes.txt', base64.encodestring(b''.join(f.readlines())))
...
{filler}{cell_phone}{email}\r\n'.format(
        ...
        f.close()
        f = open(doc_name, 'rb')
        return self.env['popup.it'].get_file('Interbank_Haberes.txt', base64.encodestring(b''.join(f.readlines())))
...
{employee_num}{user_name}{date}{inter_acc_number}\r\n'.format(
                ...
                amount_5='0000000.00',
                amount_6='0000000.00',
                acc_number=self.parse_to_txt(
                    PartnerAccount.acc_number if PartnerAccount.type_of_account in ['0', '1'] else '', 'str', range=14),
                whites=' ' * 16,
                ...
                date=self.parse_to_txt(self.payment_date, 'date'),
                inter_acc_number=self.parse_to_txt(
                    PartnerAccount.acc_number if PartnerAccount.type_of_account == '3' else '', 'str', range=20),
            ))
        f.close()
        f = open(doc_name, 'rb')
        return self.env['popup.it'].get_file('Scotiabank_Haberes.txt', base64.encodestring(b''.join(f.readlines())))
        ...
        for line in self.slip_ids:
            ...
            f.write(
                '{doc_type}{doc_number}{employee_name}{payment_way}{acc_number}{acc_number_cci}{amount}{labor_regime}{currency}{concept}{payment_type}\r\n'.format(
                    ...
                    acc_number=self.parse_to_txt(
                        PartnerAccount.acc_number if PartnerAccount.type_of_account in ['0', '1'] else '', 'str',
                        range=10),
                    acc_number_cci=self.parse_to_txt(
                        PartnerAccount.acc_number if PartnerAccount.type_of_account in ['3'] else '', 'str', range=20),
    ...
    def get_amounts_total_cts(self, Lines):
        if self.cts_dollars:
            ...
            amounts = Lines.mapped('total_cts')
        return sum(amounts)

    def get_cts_lines(self):
        if self.cts_dollars:
            Lines = self.cts_line_ids.filtered(lambda l: l.employee_id.cts_bank_account_id.currency_id.name == 'USD')
        else:
            Lines = self.cts_line_ids.filtered(lambda l: l.employee_id.cts_bank_account_id.currency_id.name == 'PEN')
        return Lines
...
{reference}{alert_indicator}{alert_val}{whites}{last_6_rem}{amount_currency}{filler}\r\n'.format(
                ...
                payment_doc=self.parse_to_txt(PartnerAccount.acc_number, 'str', range=20),
                beneficiary_name=self.parse_to_txt(line.employee_id.name, 'str', range=40),
                amount=self.parse_to_txt(line.cts_dollars if self.cts_dollars else line.total_cts, 'float', range=15,
                                         left=False),
        ...
        f.close()
        f = open(doc_name, 'rb')
        return self.env['popup.it'].get_file('BBVA_CTS.txt', base64.encodestring(b''.join(f.readlines())))
...
{beneficiary_ref}{company_ref}{currency}{amount}\r\n'.format(
                ...
                company_ref=self.parse_to_txt(self.env.company.name, 'str', range=20),
                currency='1001' if self.cts_dollars else '0001',
                amount=self.parse_to_txt(line.cts_dollars if self.cts_dollars else line.total_cts, \
                                         'float', range=17, left=False, decimal_point=True),
            ))
        f.close()
        f = open(doc_name, 'rb')
        return self.env['popup.it'].get_file('BCP_CTS.txt', base64.encodestring(b''.join(f.readlines())))
...
{filler}{cell_phone}{email}\r\n'.format(
                ...
                account_type=ACCOUNT_TYPE.get(PartnerAccount.type_of_account, ' ' * 3),
                account_currency=' ' * 2 if PartnerAccount.type_of_account == '3' else (
                    '10' if self.cts_dollars else '01'),
                account_office=' ' * 3 if PartnerAccount.type_of_account == '3' else PartnerAccount.acc_number[:3],
                acc_number=PartnerAccount.acc_number if PartnerAccount.type_of_account == '3' \
                    else self.parse_to_txt(PartnerAccount.acc_number[:3], 'str', range=20),
                person_type=self.person_type if self.person_type else ' ',
                ...
                amount_cts=self.parse_to_txt(line.cts_dollars if self.cts_dollars else line.total_cts, 'float',
                                             range=15),
                filler=' ' * 6,
                cell_phone=self.parse_to_txt(line.employee_id.work_phone or '', 'str', range=40),
                email=self.parse_to_txt(line.employee_id.work_email or '', 'str', range=140),
        ...
        f.close()
        f = open(doc_name, 'rb')
        return self.env['popup.it'].get_file('Interbank_CTS.txt', base64.encodestring(b''.join(f.readlines())))
        ...
        for line in Lines:
            ...
            f.write(
                '{type_document}{number_document}{emp_name}{payment_method}{cts_acc}{cci_acc}{rem_amount}{cts_currency}{concept}{type_payment}\r\n'.format(
                    ...
                    amount=self.parse_to_txt(line.cts_dollars if self.cts_dollars else line.total_cts, 'float',
                                             range=15),
                    cci_acc=self.parse_to_txt(
                        PartnerAccount.acc_number if PartnerAccount.type_of_account == '3' else '',
                        'str', range=20),
                    rem_amount='{0:.2f}'.format(line.total_cts).replace('.', '').zfill(11),
                    type_payment='05',  # CTS
                    concept=self.cts_id.name[0:20],
                ))
        f.close()
        f = open(doc_name, 'rb')
        return self.env['popup.it'].get_file('Scotiabank_CTS.txt', base64.encodestring(b''.join(f.readlines())))
            ...
            if amount_to_pay != 0:
                ...
                '{doc_type}{doc_number}{employee_name}{payment_way}{acc_number}{acc_number_cci}{amount}{labor_regime}{currency}{concept}{payment_type}\r\n'.format(
                    ...
                    acc_number=self.parse_to_txt(
                        ...
                        range=10),
                    acc_number_cci=self.parse_to_txt(
                        PartnerAccount.acc_number if PartnerAccount.type_of_account in ['3'] else '', 'str',
                        range=20),
                    amount=self.parse_to_txt(amount_to_pay, 'float', range=11, left=False),
                    labor_regime='1',
                    currency='00' if BankAccount.currency_id.name == 'PEN' else '01',
