...
class AccountBalanceDateRep(models.TransientModel):
	_name = 'account.balance.date.rep'
	_description = 'Account Balance Date Rep'
	...
	def get_report(self):
		...
		sql = """
			CREATE OR REPLACE view account_balance_date_book as (SELECT a1.* FROM get_saldos('%s','%s',%s,%s) a1
		...
		if self.type_show == 'pantalla':
			return {
				'name': 'Saldo por Fecha Documento',
				'type': 'ir.actions.act_window',
				'res_model': 'account.balance.date.book',
				'view_mode': 'tree',
				'view_type': 'form',
				'views': [(False, 'tree')],
			}
	...
	def get_excel(self):
		...
		workbook = Workbook(direccion +'Saldo_por_Fecha_Doc.xlsx')
		...
		worksheet = workbook.add_worksheet("Saldo por Fecha Doc")
		...
		for line in self.env['account.balance.date.book'].search([]):
		...
		workbook.close()
		f = open(direccion +'Saldo_por_Fecha_Doc.xlsx', 'rb')
		return self.env['popup.it'].get_file('Saldo por Fecha Documento.xlsx',base64.encodestring(b''.join(f.readlines())))
	...
	def get_pdf(self):
		...
		name_file = "Saldo_por_Fecha_Doc.pdf"
	...
	#Crear Tabla
		...
		totales = [0,0,0,0]
		for fila in self.env['account.balance.date.book'].search([]):
	...
	#Build
		...
		return self.env['popup.it'].get_file('Saldo por Fecha Documento.pdf',base64.encodestring(b''.join(f.readlines())))
