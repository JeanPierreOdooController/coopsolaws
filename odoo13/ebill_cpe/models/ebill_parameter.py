...
class EbillParameter(models.Model):
    ...
    print_uom = fields.Boolean(string="Imprimir Unidades de Medida en Productos",default=True)
    rounded_detraccion = fields.Boolean(string="Redondear Detracciones",default=False)
    rounded_detraccion_from = fields.Integer(string="Redondear Detracciones a Partir de",default=5)
    ...
    def change_token_test(self):
        ...
        is_purchase = self.env['ir.module.module'].search([('name', '=', 'remission_guide_it'), ('state', '=', 'installed')])
        if is_purchase:
            sql = f"UPDATE remission_guide_series SET token = '{token}' , path = '{url}'"
        ...
        sql = f"UPDATE ebill_parameter SET send_customer_email = 'f' "
        self.env.cr.execute(sql)
