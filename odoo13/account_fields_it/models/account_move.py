...
class AccountMove(models.Model):
	...
	#payment_date_it = fields.Date(string='Fecha de Pago',compute='_compute_payment_date_it_info',store=True)
	...
	#def _get_reconciled_info_date_it(self):
	#	self.ensure_one()
	...
	#		reconciled_vals.append(counterpart_line.date)
	#	return reconciled_vals

	#@api.depends('type', 'line_ids.amount_residual')
	#def _compute_payment_date_it_info(self):
	#	for move in self:
	#		if move.state != 'posted' or not move.is_invoice(include_receipts=True):
	#			move.payment_date_it = None
	#			continue
	#		reconciled_vals = move._get_reconciled_info_date_it()
	#		if reconciled_vals:
	#			date = None
	...
	@api.constrains('ref', 'type', 'partner_id', 'journal_id','type_document_id')
	def _check_duplicate_supplier_reference(self):
		moves = self.filtered(lambda move: move.type in ('in_invoice','in_refund','entry') and move.ref)
		if not moves:
			return

		self.env["account.move"].flush([
			"ref", "type", "invoice_date", "journal_id",
			"company_id", "partner_id", "commercial_partner_id","type_document_id",
		])
		...
		self._cr.execute('''
			...
			INNER JOIN account_move move2 ON
					(CASE WHEN move2.ref is not null and move2.ref LIKE '%%-%%' and move2.type in ('out_invoice', 'in_invoice', 'out_refund', 'in_refund') THEN split_part(replace(move2.ref,' ',''), '-', 1)||'-'||REPLACE(LTRIM(REPLACE(split_part(replace(move2.ref,' ',''), '-', 2), '0', ' ')),' ', '0') ELSE move2.ref END ) = (CASE WHEN move.ref is not null and move.ref LIKE '%%-%%' and move.type in ('out_invoice', 'in_invoice', 'out_refund', 'in_refund') THEN split_part(replace(move.ref,' ',''), '-', 1)||'-'||REPLACE(LTRIM(REPLACE(split_part(replace(move.ref,' ',''), '-', 2), '0', ' ')),' ', '0') ELSE move.ref END )
					...
					AND move2.type in ('in_invoice','in_refund','entry')
					AND move2.id != move.id
			LEFT JOIN account_journal journal2 ON journal2.id = move2.journal_id
			WHERE (journal2.register_sunat = journal.register_sunat AND (journal2.register_sunat = '1')) AND
			move.id IN %s
		''', [tuple(moves.ids)])
	...
	def _check_duplicate_customer_reference(self):
		moves = self.filtered(lambda move: move.type in ('out_invoice','out_refund','entry') and move.ref)
		if not moves:
			return
		...
		self._cr.execute('''
			...
			INNER JOIN account_move move2 ON
					...
					AND move2.type in ('out_invoice','out_refund','entry')
					AND move2.id != move.id
			LEFT JOIN account_journal journal2 ON journal2.id = move2.journal_id
...
class AccountMoveLine(models.Model):
	...
	account_type_it_id = fields.Many2one(related='account_id.account_type_it_id', store=True, index=True, copy=False,string='Tipo Estado Financiero')
