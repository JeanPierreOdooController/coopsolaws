...
class HrContract(models.Model):
	...
	def _check_current_contract(self):
		""" Dos contratos en estado [entrantes | abierto | cerrar] no puede superponerse """
		for contract in self.filtered(lambda c: (c.state not in ['draft','cancel'] or c.state == 'draft' and c.kanban_state == 'done') and c.employee_id):
			...
			if contract.contract_type not in ('S','O'):
				...
				if self.search_count(domain):
					raise ValidationError('Un empleado solo puede tener un contrato al mismo tiempo. (Excluyendo borrador y contratos cancelados)')
	...
	####This function help to get the first employee's contract using the situation as a conditional to get it
...
class HrWorkSuspension(models.Model):
	...
	# resumen_plani_id = fields.Many2one('hr.resumen.planilla', string='Periodo', required=True)
