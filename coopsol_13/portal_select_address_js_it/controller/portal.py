...
class CustomerPortal(CustomerPortal):
    ...
    def details_form_validate(self, data):
        ...
        #cd raise ValidationError([error,error_message])

        return error, error_message

    @http.route(['/shop/states_infos_js/<model("res.country.state"):state>'], type='json',
                auth="public", methods=['POST'],
                website=True)
    def states_infos_js(self, state, mode=None, **kw):
        country = state.country_id
        provinces = state.get_website_sale_provinces_js()
        return dict(
    ...
    @http.route(['/shop/province_infos_js/<model("res.country.state"):state>'], type='json',
                auth="public", methods=['POST'],
                website=True)
    def province_infos_js(self, state, mode=None, **kw):
        country = state.country_id
        provinces = state.get_website_sale_district_js()
        return dict(
            fields=country.get_address_fields(),
            states=[(st.id, st.name, st.code) for st in provinces],
            phone_code=country.phone_code
    ...
    @http.route(['/shop/district_js/<model("res.country.state"):state>'], type='json',
                auth="public", methods=['POST'],
                website=True)
    def district_id_js(self, state, mode=None, **kw):
    ...
    @http.route(['/shop/country_infos_js/<model("res.country"):country>'], type='json', auth="public", methods=['POST'],
                website=True)
    def country_infos(self, country, mode, **kw):
        return dict(
            fields=country.get_address_fields(),
            states=[(st.id, st.name, st.code) for st in country.get_website_sale_states_js(mode=mode)],
            phone_code=country.phone_code
        )
